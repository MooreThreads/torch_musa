...
class TestFusedAdam:
    """class of FusedAdam unit test suits"""
    ...
    def test_fp32_traing(self):
        """test FusedAdam in fp32 training mode"""
        lr = 0.001
        model = Model().to("musa")
        ...
        optimizer_ref = torch.optim.Adam(params_ref, lr=lr)
    ...
    def test_amp_traing(self):
        """Test FusedAdam in mixed precision training mode"""
        lr = 1e-4
        model = Model().to("musa")
        model_ref = Model().to("musa")
        model.load_state_dict(copy.deepcopy(model_ref.state_dict()))
