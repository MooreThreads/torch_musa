    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-6, weight_decay=0.01):
        ...
        if multi_tensor_applier.available:
            ext_module = ext_loader.load_ext(
                "_ext",
                ["multi_tensor_l2norm", "multi_tensor_lamb"]
            )
...
class TestFusedLAMB(TestLamb):
    ...
    def test_lamb_option(self):
        ...
        for wd in weight_decay:
            lamb_option = {"lr": 0.01, "betas": (0.6, 0.9), "eps": 3e-6, "weight_decay": wd}
            ref_param, tst_param, ref_optim, tst_optim = \
                self.gen_param_optim([tensor], lamb_option)
    ...
    def run(self):
        test_funcs = [m for m in dir(self) if callable(getattr(self, m)) and m.startswith("test")]
