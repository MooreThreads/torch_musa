...
def test_single_device_ops():
    ...
    torch_musa.empty_cache()
    torch_musa.reset_peak_stats()
    torch_musa.set_device(1) # all tests are on device 1
    input_data_small = torch.randn(1024 * 4, 1024 * 4).to("musa") # 16 * 1024 * 1024 * 4 = 64MB
    input_data_large = torch.randn(1024 * 8, 1024 * 8).to("musa") # 64 * 1024 * 1024 * 4 = 256MB
    del input_data_large # free 256MB on device 1
    summary = torch_musa.memory_summary(0)
    assert "reserved" in summary
    summary = torch_musa.memory_summary(1)
    print(summary)
    assert "reserved" in summary

    stats = torch_musa.memory_stats(0)
    assert isinstance(stats, dict)
    stats = torch_musa.memory_stats(1)
    assert isinstance(stats, dict)
    ...
    assert allocated == 0
    assert reserved == 0
    m_allocated = torch_musa.max_memory_allocated(1)
    m_reserved = torch_musa.max_memory_reserved(1)
    allocated = torch_musa.memory_allocated(1)
    reserved = torch_musa.memory_reserved(1)
...
def test_all_devices_ops():
    ...
    tensor_0 = torch.randn(1024 * 4, 1024 * 4).to("musa") # 16 * 1024 * 1024 * 4 = 64MB
    torch_musa.set_device(1)
    tensor_1 = torch.randn(1024 * 8, 1024 * 8).to("musa") # 64 * 1024 * 1024 * 4 = 256MB
