...
class RawSDP(torch.nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None, dropout_p=0.0, is_casual=False):
        ...
        _, _, q_seq_len, head_dim = query.shape
...
def sdp_func(query, key, value, attn_mask=None, dropout_p=0.0, is_casual=False):
    ...
    if attn_mask.shape == (batch_size, seq_len) and attn_mask.is_cpu:
...
def function(input_data, func, train=False):
    ...
    # Warning: mudnn MHA numerical untability, have to set abs_diff=1e-4
    comparator = DefaultComparator(abs_diff=1e-4)
...
def test_sdp(case, dtype, func, mask_type, is_self_attn):
...
def test_sdp_backward(case, dtype, func, mask_type, is_self_attn):
    input_data = gen_input_data(
        case, mask_type, dtype, is_self_attention=is_self_attn)
