...
def test_masks_to_boxes(size, dtype):
    ...
    test = testing.OpTest(func=torchvision.ops.masks_to_boxes, input_args=input_args)
    test.check_result()
...
def test_nms(num_boxes, iou_threshold, dtype):
    ...
    input_args["scores"] = scores.to(dtype)
    input_args["iou_threshold"] = iou_threshold
    test = testing.OpTest(func=torchvision.ops.nms, input_args=input_args)
    test.check_result()
...
def test_roi_align(dtype, spatial_scale, sampling_ratio, aligned):
    ...
    n_channels = 2
    num_imgs = 1
    x = torch.randint(50, 100, size=(num_imgs, n_channels, img_size, img_size)).to(
        dtype
    )
    ...
    test = testing.OpTest(func=torchvision.ops.roi_align, input_args=input_args)
    test.check_result()
...
def test_ps_roi_align(dtype, spatial_scale, sampling_ratio):
    ...
    input_args["spatial_scale"] = spatial_scale
    input_args["sampling_ratio"] = sampling_ratio
    test = testing.OpTest(func=torchvision.ops.ps_roi_align, input_args=input_args)
    test.check_result()
...
def test_box_area(dtype):
    ...
    input_args = {}
    input_args["boxes"] = rois.to(dtype)
    test = testing.OpTest(func=torchvision.ops.box_area, input_args=input_args)
    test.check_result()
...
def test_clip_boxes_to_image(dtype, size):
    ...
    test = testing.OpTest(
        func=torchvision.ops.clip_boxes_to_image, input_args=input_args
    )
    test.check_result()
...
def test_generalized_box_iou(dtype):
    ...
    test = testing.OpTest(
        func=torchvision.ops.generalized_box_iou, input_args=input_args
    )
    test.check_result()
...
def test_remove_small_boxes(dtype):
    ...
    test = testing.OpTest(
        func=torchvision.ops.remove_small_boxes, input_args=input_args
    )
    test.check_result()
...
def test_complete_box_iou_loss(dtype, reduction, eps):
    ...
    test = testing.OpTest(
        func=torchvision.ops.complete_box_iou_loss, input_args=input_args
    )
    test.check_result()
...
def test_distance_box_iou_loss(dtype, reduction, eps):
    ...
    test = testing.OpTest(
        func=torchvision.ops.distance_box_iou_loss, input_args=input_args
    )
    test.check_result()
...
def test_generalized_box_iou_loss(dtype, reduction, eps):
    ...
    test = testing.OpTest(
        func=torchvision.ops.generalized_box_iou_loss, input_args=input_args
    )
    test.check_result()
...
def test_frozen_batch_norm2d(num_features, dtype, eps):
    m = torchvision.ops.FrozenBatchNorm2d(num_features=num_features, eps=eps)
...
def test_mlp(in_channels, hidden_channels, dtype):
    m = torchvision.ops.MLP(in_channels=in_channels, hidden_channels=hidden_channels)
    ...
    assert torch.allclose(output_cpu, output_musa.to("cpu"), atol=1e-3)
...
def test_permute(dtype, dims):
    m = torchvision.ops.Permute(dims=dims)
...
def test_squeeze_excitation(input_channels, squeeze_channels, dtype):
    m = torchvision.ops.SqueezeExcitation(
        input_channels=input_channels, squeeze_channels=squeeze_channels
    )
...
def test_stochastic_depth(p, dtype, mode, training):
    ...
    test = testing.OpTest(func=torchvision.ops.stochastic_depth, input_args=input_args)
    test.check_result()
...
def test_deform_conv2d(dtype):
    ...
    input_args["offset"] = torch.rand(4, 2 * kernel_height * kernel_width, 8, 8).to(
        dtype
    )
    input_args["mask"] = torch.rand(4, kernel_height * kernel_width, 8, 8).to(dtype)

    test = testing.OpTest(func=torchvision.ops.deform_conv2d, input_args=input_args)
    test.check_result()
...
def test_drop_block2d(p, dtype, block_size):
    ...
    test = testing.OpTest(func=torchvision.ops.drop_block2d, input_args=input_args)
    test.check_result()
...
def test_drop_block3d(p, dtype, block_size):
    ...
    input_args["block_size"] = block_size
    test = testing.OpTest(
        func=torchvision.ops.drop_block3d,
