...
def test_repeat_interleave(input_data, dtype):
    ...
    output_musa = torch.repeat_interleave(input_data.to("musa"),
                                    repeat_tensor.to("musa"), dim=0)
    assert pytest.approx(output_cpu, 1e-6) == output_musa.to("cpu")

    output_size = sum(repeat_tensor)
    output_cpu = torch.repeat_interleave(input_data, repeat_tensor, dim=0, output_size=output_size)
    output_musa = torch.repeat_interleave(input_data.to("musa"),
                                    repeat_tensor.to("musa"), dim=0, output_size=output_size)
    assert pytest.approx(output_cpu, 1e-6) == output_musa.to("cpu")
