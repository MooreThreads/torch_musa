...
def test_amp_autocast():
    scale = torch.tensor([65536.], device='musa')
    growth_tracker = torch.tensor([0],device='musa',dtype=torch.int32)
    found_inf_combined = torch.tensor([1.], device='musa')
    growth_factor=2.0
    backoff_factor=0.5
    growth_interval=2000

    torch._amp_update_scale_(scale, growth_tracker, found_inf_combined,
            growth_factor, backoff_factor, growth_interval)
    testing.DefaultComparator(scale, scale/2)

    found_inf_combined = torch.tensor([0.], device='musa')
    torch._amp_update_scale_(scale, growth_tracker, found_inf_combined,
            growth_factor, backoff_factor, growth_interval)
