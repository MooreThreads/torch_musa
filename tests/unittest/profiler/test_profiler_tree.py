# pylint: disable=invalid-name, missing-class-docstring, missing-function-docstring, missing-module-docstring, unused-argument, unnecessary-dunder-call, pointless-statement, unused-import, line-too-long
...
ALLOW_MUSA_FAILURE = (torch.version.hip is not None) or IS_WINDOWS
...
class TestProfilerTree(TestCase):
    ...
    @ProfilerTree.test
    def test_profiler_experimental_tree_musa(self):
        with torch.profiler.profile(profile_memory=True) as p:
            weight = torch.ones(1, device="musa", requires_grad=True)
            x = torch.ones(1, device="musa")
            ...
            Optimizer.step#SGD.step
              ...
              aten::add_
                musaLaunchKernel
                  void at::native::vectorized_elementwise_kernel<...>(...)
            [memory]""",  # noqa: B950
            allow_failure=ALLOW_MUSA_FAILURE,
        )

    @unittest.skip("https://github.com/pytorch/pytorch/issues/83606")
    @unittest.skipIf(not torch.musa.is_available(), "MUSA is required")
    @ProfilerTree.test
    def test_profiler_experimental_tree_musa_with_stream(self):
        streams = [torch.musa.Stream() for _ in range(3)]
        results = []
        with torch.profiler.profile(profile_memory=True) as p:
            x = torch.ones((4, 4), device="musa")
    ...
    def test_profiler_experimental_tree_cuda_detailed(self):
        ...
        self.assertTreesMatch(
            ProfilerTree.format(p.profiler, 12),
            """\
            test_profiler_tree.py(...): test_profiler_experimental_tree_musa_detailed
              torch/profiler/profiler.py(...): __enter__
                ...
