...
from torch_musa.testing import MULTIGPU_AVAILABLE
...
class TestProfilerMUSA(TestCase):
    ...
    def test_mem_leak(self):
        ...
        # with MUSA events leaking the increase in memory was ~7 MB between
    ...
    @unittest.skip("Not implemented emit_mttx function.")
...
class TestRecordFunction(TestCase):
    ...
    def test_datapipe_delegation_with_profiler(self):
        class IDPIterator(torch.utils.data.IterDataPipe):
            """Iterator for Iterable Data Pipe."""
...
class TestExecutionGraph(TestCase):
    ...
    def payload(self, use_musa=False):
        u = torch.randn(3, 4, 5, requires_grad=True)
        with record_function("## TEST 1 ##", "1, 2, 3"):
            inf_val = float("inf")
...
class TestProfiler(TestCase):
        ...
        for evt in prof.events():
            if evt.device_type != DeviceType.CPU:
                if evt.device_index == 0:
                    ...
                    found_1 = True
            if "musa" in evt.name.lower() and evt.device_type == DeviceType.CPU:
                found_musa = True

        self.assertTrue(found_0)
        self.assertTrue(found_1)
        self.assertTrue(found_musa)
    ...
    def test_memory_profiler(self):
        ...
        def create_musa_tensor():
            return torch.rand(10, 10).musa()
    ...
    def test_oom_tracing(self):
        ...
        if torch.musa.is_available():
            with TemporaryFileName(mode="w+") as fname:
                prof = run_profiler(create_musa_tensor_oom)
                check_trace(fname)
    ...
    def test_high_level_trace(self):
        ...
        try:
            ...
            raise AssertionError("Expected no exception without profiling.") from exc
    ...
    def test_flops(self):
        ...
        if not (kineto_available() and torch.musa.is_available()):
            return
    ...
    def test_kineto_profiler_api(self):
        ...
        for step, test_schedule_expected_output in enumerate(
            test_schedule_expected_outputs
        ):
            self.assertEqual(test_schedule(step), test_schedule_expected_output)
    ...
    def _test_profiler_tracing(self, use_kineto):
        ...
        with TemporaryFileName(mode="w+") as fname:
            prof.export_chrome_trace(fname)
            # read the trace and expect valid json if the JSON generated by export_chrome_trace
            # is not valid, this will throw and fail the test.
            with io.open(fname, "r", encoding="UTF-8") as f:
                json.load(f)
...
class TestTorchTidyProfiler(TestCase):
    ...
    def test_allocations(self):
        ...
        # total_reserved is only for MUSACachingAllocator
...
class TestExperimentalUtils(TestCase):
    ...
    def generate_mock_profile():
        musa_events = [
            MockKinetoEvent("musaLaunchKernel", 400, 100, 1, 0),
            MockKinetoEvent("musaLaunchKernel", 500, 100, 2, 0),
            MockKinetoEvent("musaLaunchKernel", 600, 100, 3, 0),
            MockKinetoEvent("musaLaunchKernel", 700, 100, 4, 0),
            MockKinetoEvent("musaLaunchKernel", 800, 100, 5, 0),
            MockKinetoEvent("musaLaunchKernel", 1500, 100, 6, 0),
        ...
        cpu_events = [
            MockProfilerEvent("CPU (Before musaLaunchKernel)", 1, 0, 100000),
            MockProfilerEvent("CPU (Before musaLaunchKernel)", 2, 100000, 100000),
            MockProfilerEvent("CPU (Before musaLaunchKernel)", 3, 200000, 100000),
            MockProfilerEvent("CPU (Before musaLaunchKernel)", 4, 300000, 100000),
            MockProfilerEvent("CPU (After musaLaunchKernel)", 5, 400000, 100000),
            MockProfilerEvent("CPU (After musaLaunchKernel)", 6, 500000, 100000),
            MockProfilerEvent("CPU (After musaLaunchKernel)", 7, 600000, 100000),
            MockProfilerEvent("CPU (After musaLaunchKernel)", 8, 700000, 100000),
        ...
        profiler.kineto_results.events = unittest.mock.Mock(return_value=musa_events)
    ...
    def load_mock_profile():
        ...
        if accept and torch.musa.is_available():
            ...
            kineto_events = [
                {
                    ...
                    "_device_type": 1 if e.device_type() != DeviceType.CPU else 0,
                }
        ...
        musa_events = [MockKinetoEvent(*event.values()) for event in kineto_events]
        cpu_events = []
        id_map = {}
    ...
    def test_utils_compute_queue_depth(self):
        ...
        self.assertExpectedInline(
            format_queue_depth(
                basic_evaluation.queue_depth_list, basic_evaluation.musa_events
            ),
            """\
1 [musaLaunchKernel]
2 [musaLaunchKernel]
...
""",
        ...
        self.assertExpectedInline(
            ...
            """\
0 [CPU (Before musaLaunchKernel)]
0 [CPU (Before musaLaunchKernel)]
0 [CPU (Before musaLaunchKernel)]
0 [CPU (Before musaLaunchKernel)]
1 [CPU (After musaLaunchKernel)]
2 [CPU (After musaLaunchKernel)]
3 [CPU (After musaLaunchKernel)]
4 [CPU (After musaLaunchKernel)]
5 [CPU (After GPU)]
4 [CPU (After GPU)]
2 [CPU (After GPU)]
1 [CPU (After GPU)]
""",
    ...
    def test_utils_compute_queue_depth_when_no_musa_events(self):
    ...
    def test_utils_compute_idle_time(self):
        ...
        self.assertExpectedInline(
            expected_output,
            """\
100000 [CPU (Before musaLaunchKernel)]
100000 [CPU (Before musaLaunchKernel)]
100000 [CPU (Before musaLaunchKernel)]
100000 [CPU (Before musaLaunchKernel)]
0 [CPU (After musaLaunchKernel)]
0 [CPU (After musaLaunchKernel)]
0 [CPU (After musaLaunchKernel)]
0 [CPU (After musaLaunchKernel)]
0 [CPU (After GPU)]
...
100000 [CPU (After GPU)]""",
    ...
    def test_utils_get_optimizable_events(self):
        ...
        self.assertExpectedInline(
            expected_output,
            """\
<built-in function _musa_synchronize>
aten::copy_""",
        )
...
aten::mm""",
    ...
    @unittest.skip(
        "tensor's `to` method in torch_musa  is hacked so the test is affected."
    )
    @unittest.skipIf(
        TEST_WITH_CROSSREF, "crossref intercepts calls and changes the callsite."
    )
    @unittest.skipIf(not torch.musa.is_available(), "MUSA is required")
    def test_profiler_extra_musa_copy_pattern(self):
        cases = (
            (0, lambda: torch.ones((100, 100), device="musa")),
            (1, lambda: torch.ones((100, 100)).to("musa")),
            (1, lambda: torch.zeros((100, 100)).to("musa")),
            (1, lambda: torch.empty((100, 100)).fill_(5).to("musa")),
            (1, lambda: torch.ones((100, 100)).musa()),
            (1, lambda: torch.zeros((100, 100)).musa()),
            (1, lambda: torch.empty((100, 100)).fill_(5).musa()),
            (1, lambda: torch.rand((100, 100)).musa()),
            (1, lambda: torch.randn((100, 100)).musa()),
            (1, lambda: torch.full((100, 100), 10).musa()),
            (0, lambda: torch.rand((100, 100)).to(dtype=torch.float16)),
            (0, lambda: torch.rand((100, 100)).half()),
            (0, lambda: torch.rand((100, 100), device="musa").half()),
        )
        num_matched = []
        for _, fn in cases:
    ...
    def test_profiler_fp32_matmul_pattern(self):
        x = torch.ones((100, 100), device="musa")
        with profile(with_stack=True) as prof:
            x = x @ x
    ...
    @unittest.skipIf(not torch.musa.is_available(), "MUSA is required")
    def test_profiler_extra_musa_copy_pattern_benchmark(self):
        with profile(with_stack=True, record_shapes=True) as prof:
    ...
    def test_profiler_matmul_dim_fp16_pattern(self):
        cases = (
            (1, torch.randn((201, 201), device="musa", dtype=torch.float16)),
            (1, torch.randn((3, 97, 97), device="musa", dtype=torch.float16)),
            (0, torch.randn((200, 200), device="musa", dtype=torch.float16)),
            (0, torch.randn((3, 200, 200), device="musa", dtype=torch.float16)),
        )
        num_matched = []
        for _, x in cases:
            with profile(with_stack=True, record_shapes=True) as prof:
    ...
    def test_profiler_pattern_matcher_json_report(self):
        ...
        try:
            with open("./torchtidy_report.json", encoding="UTF-8") as f:
                report = json.load(f)
