...
if __name__ == '__main__':
    ...
    parser.add_argument("--scan-dir", help="Specify directory to be scanned",
                        default="/home/torch_musa/torch_musa/csrc/aten/ops")
    ...
    parser.add_argument("--regex", help="Specify search regular expression",
                        default=r"m\.impl\(\s*?TORCH_SELECTIVE_NAME\(\s*?\"(.*?)\"|m\.impl\("
                                r"\s*?\"(.*?)\"|ADVANCED_REGISTER\(\s*?aten,\s*?PrivateUse1,"
                                r"\s*?\"(.*?)\",")
    parser.add_argument("--output-path", help="Specify ops' scanning result location",
                        default="./torch_musa_full_ops.xlsx")

    args = parser.parse_args()
    test_dir = args.scan_dir
    test_file_ext = args.file_ext
    test_ops_regex = args.regex
    ops_scanner = ops_scanner_base.OpsScannerBase(test_dir, test_file_ext, test_ops_regex,
