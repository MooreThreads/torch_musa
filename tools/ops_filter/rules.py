...
def is_deprecated(torch_op: dict) -> bool:
    ...
    return (
        op_name.startswith("_cast_")
        or op_name == "set_data"
        or op_name.startswith("_aminmax.dim")
        or op_name.startswith("nuclear_norm")
    )
...
def is_other_conv(torch_op: dict) -> bool:
    ...
    if op_name in ["convolution_overrideable", "convolution_backward_overrideable"]:
        return False
    return (
        op_name.startswith("convolution")
        or op_name.startswith("_convolution")
        or op_name.startswith("conv1d")
        or op_name.startswith("conv2d")
        or op_name.startswith("conv3d")
        or op_name.startswith("conv_tbc")
        or op_name.startswith("conv_transpose")
        or op_name.startswith("slow_conv")
        or op_name.startswith("_slow_conv")
    )


def is_other_attention(torch_op: dict) -> bool:
    op_name = torch_op["func"]
    return op_name == "_fused_sdp_choice" or "_attention" in op_name
