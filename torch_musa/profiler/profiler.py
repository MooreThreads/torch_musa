"""Torch musa profiler definition."""
...
def supported_activities():
    ...
    Note: profiler uses MUPTI library to trace on-device MUSA kernels.
    In case when MUSA is enabled but MUPTI is not available, passing
    ``ProfilerActivity.MUSA`` to profiler results in using the legacy MUSA
    profiling code (same as in the legacy ``torch.autograd.profiler``).
    This, in turn, results in including MUSA time in the profiler table output,
...
class _KinetoProfile:
    ...
    Args: activities (iterable): list of activity groups (CPU, MUSA) to use in profiling,
    supported values: ``torch.profiler.ProfilerActivity.CPU``,
    ``torch.profiler.ProfilerActivity.MUSA``. Default value: ProfilerActivity.CPU and (when
    available) ProfilerActivity.MUSA. record_shapes (bool): save information about operator's
    input shapes. profile_memory (bool): track tensor memory allocation/deallocation. with_stack
    (bool): record source information (file and line number) for the ops. with_flops (bool): use
    formula to estimate the FLOPS of specific operators (matrix multiplication and 2D
    convolution). with_modules (bool): record module hierarchy (including function names)
    corresponding to the callstack of the op. e.g. If module A's forward call's module B's
    forward which contains an aten::add op, then aten::add's module hierarchy is A.B Note that
    this support exist, at the moment, only for TorchScript models and not eager mode models.
    ...
    .. note::
        ...
        Enabling shape and stack tracing results in additional overhead. When record_shapes=True
        is specified, profiler will temporarily hold references to the tensors; that may further
        prevent certain optimizations that depend on the reference count and introduce extra
        tensor copies.
    """
    ...
    def prepare_trace(self):
        ...
        self.profiler = prof.profile(
            use_musa=(ProfilerActivity.MUSA in self.activities),
    ...
    def export_chrome_trace(self, path: str):
        ...
        if path.endswith(".gz"):
            ...
            file_handle.close()
            retvalue = self.profiler.export_chrome_trace(file_handle.name)
    ...
    def export_stacks(self, path: str, metric: str = "self_cpu_time_total"):
        ...
        Args:
            ...
            metric (str): metric to use: "self_cpu_time_total" or "self_musa_time_total"
...
def schedule(
    *, wait: int, warmup: int, active: int, repeat: int = 0, skip_first: int = 0
    ...
    the first ``skip_first`` steps, then wait for ``wait`` steps, then do the warmup for the next
    ``warmup`` steps, then do the active recording for the next ``active`` steps and then repeat
    the cycle starting with ``wait`` steps. The optional number of cycles is specified with the
    ``repeat`` parameter, the zero value means that the cycles will continue until the profiling
    is finished.
    """
...
def tensorboard_trace_handler(
    ...
    def handler_fn(prof_tool) -> None:
        ...
        file_name = f"{worker_name}.{int(time.time() * 1000)}.pt.trace.json"
        if use_gzip:
            file_name = file_name + ".gz"
        prof_tool.export_chrome_trace(os.path.join(dir_name, file_name))
...
class profile(_KinetoProfile):
    ...
    supported values: ``torch.profiler.ProfilerActivity.CPU``,
    ``torch.profiler.ProfilerActivity.MUSA``. Default value: ProfilerActivity.CPU and (when
    available) ProfilerActivity.MUSA. schedule (Callable): callable that takes step (int) as a
    single parameter and returns ``ProfilerAction`` value that specifies the profiler action to
    perform at each step. on_trace_ready (Callable): callable that is called at each step when
    ``schedule`` returns ``ProfilerAction.RECORD_AND_SAVE`` during the profiling. record_shapes (
    bool): save information about operator's input shapes. profile_memory (bool): track tensor
    memory allocation/deallocation. with_stack (bool): record source information (file and line
    number) for the ops. with_flops (bool): use formula to estimate the FLOPs (floating point
    operations) of specific operators (matrix multiplication and 2D convolution). with_modules (
    bool): record module hierarchy (including function names) corresponding to the callstack of
    the op. e.g. If module A's forward call's module B's forward which contains an aten::add op,
    then aten::add's module hierarchy is A.B Note that this support exist, at the moment,
    only for TorchScript models and not eager mode models. experimental_config (
    _ExperimentalConfig) : A set of experimental options used for Kineto library features. Note,
    ...
    .. note:: Use :func:`~torch.profiler.tensorboard_trace_handler` to generate result files for
    TensorBoard:
        ...
        TensorBoard Plugin <https://github.com/pytorch/kineto/tree/master/tb_plugin>`__

    .. note:: Enabling shape and stack tracing results in additional overhead. When
    record_shapes=True is specified, profiler will temporarily hold references to the tensors;
    that may further prevent certain optimizations that depend on the reference count and
    introduce extra tensor copies.
    ...
    def __init__(
        ...
        *,
        ...
        if use_musa is not None:
            ...
            if use_musa:
                activities_set.add(ProfilerActivity.MUSA)
            elif ProfilerActivity.MUSA in activities_set:
                activities_set.remove(ProfilerActivity.MUSA)
        ...
        self.action_map: Dict[
            ...
            # key is (prev_action, current_action), value is action list corresponding to the
            # state pair.
