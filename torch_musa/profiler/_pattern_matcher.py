...
class Pattern:
    ...
    def benchmark_summary(self, events: List[_ProfilerEvent]):
        ...
        return (
            f"{self.name}: {len(events)} events matched. Total Estimated Speedup: "
            f"{format_time(original_time - new_time)} ({round(original_time / new_time, 2)}X) "
        )
...
class ExtraMUSACopyPattern(Pattern):
    ...
    def __init__(self, prof: profile, should_benchmark: bool = False):
        ...
        self.description = "Filled a CPU tensor and immediately moved it to GPU. Please " \
                           "initialize it on GPU. "
        self.url = "https://pytorch.org/tutorials/recipes/recipes/tuning_guide.html#create" \
...
class ForLoopIndexingPattern(Pattern):
    ...
    def match(self, event: _ProfilerEvent):
        ...
        if event.id in self.visited:
            return False
        repeat_count = 1
        _, next_event = self.siblings_of(event)
        if len(next_event) <= 1:
            return False
        ...
        next_select_idx = index_of_first_match(next_event, lambda e: e.name == "aten::select")
        if next_select_idx is None:
            return False
        indexing_ops = [event] + next_event[:next_select_idx]
        next_event = next_event[len(indexing_ops) - 1:]
        for i in range(0, len(next_event), len(indexing_ops)):
            if same_ops(indexing_ops, next_event[i: i + len(indexing_ops)]):
                repeat_count += 1
                self.visited.add(next_event[i].id)
            else:
                break
...
class FP32MatMulPattern(Pattern):
    ...
    def __init__(self, prof: profile, should_benchmark: bool = False):
        ...
        self.url = "https://pytorch.org/docs/stable/notes/musa.html#tensorfloat-32-tf32-on-ampere" \
                   "-devices "
...
class SynchronizedDataLoaderPattern(Pattern):
    ...
    def __init__(self, prof: profile, should_benchmark: bool = False):
        ...
        self.description = (
            ...
            "Please enable asynchronous dataloading by setting num_workers > 0 when initializing "
            "DataLoader. "
        )
        self.url = (
...
class Conv2dBiasFollowedByBatchNorm2dPattern(Pattern):
    ...
    def __init__(self, prof: profile, should_benchmark: bool = False):
        ...
        self.description = "Detected bias enabled in Conv2d that is followed by BatchNorm2d. " \
                           "Please set 'bias=False' in Conv2d. "
        self.url = (
            "https://pytorch.org/tutorials/recipes/recipes/tuning_guide.html"
...
class MatMulDimInFP16Pattern(Pattern):
    """Matrix Multiplication Dimension Not Aligned Pattern."""

    def __init__(self, prof: profile, should_benchmark: bool = False):
        super().__init__(prof, should_benchmark)
        self.name = "Matrix Multiplication Dimension Not Aligned Pattern"
        self.description = "Detected matmul with dimension not aligned. Please use matmul with " \
                           "aligned dimension. "
        self.url = "https://pytorch.org/tutorials/recipes/recipes/tuning_guide.html#use-mixed" \
                   "-precision-and-amp "
...
def source_code_location(event: Optional[_ProfilerEvent]):
    while event:
        if event.tag in (_EventType.PyCall, _EventType.PyCCall):
            assert isinstance(event.extra_fields, (_ExtraFields_PyCCall, _ExtraFields_PyCall))
            if not event.extra_fields.caller.file_name.startswith("torch" + os.sep):
                return f"{event.extra_fields.caller.file_name}:" \
                       f"{event.extra_fields.caller.line_number} "
        event = event.parent
