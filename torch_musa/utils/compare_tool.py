...
def compare_for_single_op(inputs_data_save_path, op_func, atol, rtol):
    ...
    with torch.serialization.safe_globals([slice]):
        inputs_data = torch.load(inputs_data_save_path, weights_only=True)
...
def save_data_for_op(out, args, kwargs, save_dir, op_name, file_suffix=""):
    ...
    None
    """
    inputs_save_path = os.path.join(save_dir, f"{op_name}_inputs{file_suffix}.pt")
    outputs_save_path = os.path.join(save_dir, f"{op_name}_outputs{file_suffix}.pt")
    ...
    with open(inputs_save_path, "wb") as f:
        torch.save(inputs_data, f)
    with open(outputs_save_path, "wb") as f:
        torch.save(outputs_data, f)

    print(
        f"Input data is saved to {inputs_save_path},\n"
        f"output data is saved to {outputs_save_path}"
    )
