...
class SimplePortingViaMusify:
    ...
    def __init__(self, cuda_dir_path: str, ignore_patterns: List[str],
                 extra_mapping: Dict[str, str] = None, drop_default_mapping: bool = False,
                 mapping: List[str] = None,
                 output_method: str = "inplace", direction: str = "c2m",
                 log_level: str = "INFO"):
    ...
    def _copy_srcs(self):
        shutil.copytree(self.cuda_dir_path, self.musa_dir_path,
                        ignore=shutil.ignore_patterns(*self.ignore_patterns),
                        dirs_exist_ok=True)
        LOGGER.info("Finish copying sources from %s to %s and ignore %s.",
                    self.cuda_dir_path, self.musa_dir_path, self.ignore_patterns)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cuda-dir-path", help="Specify cuda directory to port, "
                                                "e.g. /home/mmcv/mmcv/ops/csrc",
                        default="/home/mmcv/mmcv/ops/csrc")
    parser.add_argument("--ignore-patterns", help="Specify cuda directory to port, "
                                                  "e.g. [\"*parrots*\", \"*mlu*\", \"*npu*\", "
                                                  "\"*mps*\"]",
                        default=["*parrots*", "*mlu*", "*npu*", "*mps*"])
    parser.add_argument("--extra-mapping", help="Specify mapping rule"
                                                "e.g. {\"cuda\":\"musa\"}",
                        default={})
    parser.add_argument("--drop-default-mapping", action='store_true',
                        help="Specify whether to drop default mapping")
    parser.add_argument("--mapping", help="Specify where extra mapping files locate"
                                          "e.g. ['mapping.json']",
                        default=glob.glob(join(SEFL_PATH, "mapping", "*.json")))
    parser.add_argument("--output-method", help="Specify output method. e.g. ",
                        choices=["terminal", "create", "inplace"], default="inplace")
    parser.add_argument("--direction", help="convert direction", choices=["c2m", "m2c"],
                        default="c2m")
    parser.add_argument("--log-level", help="lowest log level to display",
                        choices=["DEBUG", "INFO", "WARNING"], default="INFO")
    args = parser.parse_args()
    print(vars(args))
    simple_porting_test = SimplePortingViaMusify(args.cuda_dir_path,
