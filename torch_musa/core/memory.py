...
def memory_stats(device=None):
    ...
    device = device if device else torch_musa.current_device()
...
def memory_stats_as_nested_dict(device=None):
    ...
    return torch_musa._MUSAC._musa_memoryStats()
...
def memory_summary(device: Union[Device, int] = None, abbreviated: bool = False) -> str:
    ...
    lines = []
    lines.append("=" * 75)
    lines.append(" {_:16} PyTorch MUSA memory summary, device ID {device:<17d} ")
    lines.append("-" * 75)
    lines.append(
