...
def _validate_musa_device(location: str) -> None:
    ...
    device_idx = torch._utils._get_device_index(torch.device(location),
                                                optional=True, allow_cpu=False) or 0

    if not torch_musa.is_available():
        raise RuntimeError('Attempting to deserialize object on a MUSA '
    ...
    if device_idx >= device_count:
        raise RuntimeError('Attempting to deserialize object on MUSA device '
...
def _musa_deserialize(obj: Any, location: str) -> Any:
    ...
    if location.startswith('musa'):
        ...
        if obj.device.type != 'cpu':
            ...
            return torch.UntypedStorage(obj.size(), device=torch.device(location)).copy_(obj, False)
        return obj
    return None
...
def register_deserialization() -> None:
    ...
    torch.serialization.register_package(_REGISTRATION_KEY, _musa_tag, _musa_deserialize)
