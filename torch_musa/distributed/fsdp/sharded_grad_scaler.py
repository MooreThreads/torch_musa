...
from torch_musa.core.amp.grad_scaler import _MultiDeviceReplicator, GradScaler, OptState
...
def _is_supported_device(tensor: torch.Tensor) -> bool:
    return tensor.device.type in ("musa", "cpu")
...
class ShardedGradScaler(GradScaler):
    ...
    * Sync inf/nan for scaled gradient tensors on any torch.device (where tensors are placed) across
    ...
    def unscale_(self, optimizer: torch.optim.Optimizer) -> None:
        ...
        # TODO(mt-ai): not sure if there is an impact on MUSA
        if self._scale.device.type == "musa":
        ...
        for v in optimizer_state["found_inf_per_device"].values():
            if v.device.type == "cpu":
                ...
                future_handles.append(
                    dist.all_reduce(
                        v_on_musa, async_op=True, group=self.process_group
                    ).get_future()
                )
                v.copy_(v_on_musa.cpu())
            else:
                future_handles.append(
                    dist.all_reduce(
    ...
    def update(self, new_scale: Optional[Union[float, torch.Tensor]] = None) -> None:
        ...
        Args:
            new_scale (float or :class:`torch.musa.FloatTensor`, optional, default=None):  New scale factor.
        .. warning::
        ...
        if new_scale is not None:
            ...
            if isinstance(new_scale, float):
                ...
                reason = "new_scale should be a float or a 1-element torch.musa.FloatTensor with requires_grad=False."
                assert isinstance(new_scale, torch.musa.FloatTensor), reason  # type: ignore[attr-defined]
