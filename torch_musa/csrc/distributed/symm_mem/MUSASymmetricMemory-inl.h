...
signal_cas(uint32_t* addr, uint32_t compare, uint32_t val) {
#if defined(USE_ROCM)
...
template <std::memory_order Sem>
__device__ __forceinline__ void signal_release_fence() {
#if !defined(USE_ROCM)
  if constexpr (
      Sem == std::memory_order_release || Sem == std::memory_order_acq_rel ||
      Sem == std::memory_order_seq_cst) {
...
template <std::memory_order Sem>
__device__ __forceinline__ void signal_acquire_fence() {
#if !defined(USE_ROCM)
  if constexpr (
      Sem == std::memory_order_acquire || Sem == std::memory_order_acq_rel ||
      Sem == std::memory_order_seq_cst) {
    __threadfence_system();
  }
#endif
...
#else
  return static_cast<size_t>(__musa_memtime64());
#endif
}
...
__device__ __forceinline__ bool try_put_signal(
  ...
  signal_release_fence<Sem>();
  while (signal_cas<Sem>(addr, 0, 1) != 0) {
...
__device__ __forceinline__ bool try_wait_signal(
    ...
    size_t timeout_ms) {
  size_t deadline = global_timer_ns() + timeout_ms * ns_per_ms;
  while (signal_cas<Sem>(addr, 1, 0) != 1) {
    if (timeout_ms != 0 && global_timer_ns() > deadline) {
      return false;
    }
  }
  signal_acquire_fence<Sem>();
  return true;
}
...
#else
  auto res = __hadd2(
      *reinterpret_cast<__mt_bfloat162*>(&a),
      *reinterpret_cast<__mt_bfloat162*>(&b));
