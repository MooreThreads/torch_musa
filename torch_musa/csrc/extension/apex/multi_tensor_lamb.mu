...
std::tuple<at::Tensor, at::Tensor> multi_tensor_l2norm_musa(
    int chunk_size,
    at::Tensor noop_flag,
...
struct LAMBStage1Functor {
  __device__ __forceinline__ void operator()(
    ...
    float clipped_global_grad_norm = (*global_grad_norm) > max_global_grad_norm
...
void multi_tensor_lamb_musa(
  ...
  std::vector<std::vector<at::Tensor>> grad_list(
      tensor_lists.begin(), tensor_lists.begin() + 1);
  std::vector<std::vector<at::Tensor>> param_list(
      tensor_lists.begin() + 1, tensor_lists.begin() + 2);

  // Compute per tensor param norm
  auto param_norm_tuple =
      multi_tensor_l2norm_musa(chunk_size, noop_flag, param_list, true);
  ...
  DISPATCH_FLOAT_AND_HALF(tensor_lists[0][0].scalar_type(),
                          0,
                          "lamb_stage_1",
  ...
  auto update_norm_tuple =
      multi_tensor_l2norm_musa(chunk_size, noop_flag, grad_list, true);
