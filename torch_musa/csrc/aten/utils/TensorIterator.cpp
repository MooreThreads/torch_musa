...
void MusaTensorIterator::_set_output_raw_strided(
  ...
  if (!base.defined()) {
    create_output_raw_strided_no_check(op, sizes, strides, options);
...
void MusaTensorIterator::compute_types(const TensorIteratorConfig& config) {
  ...
  for (const auto i : c10::irange(ntensors())) {
    ...
    if (config.cast_common_dtype_to_outputs_ && op.is_output &&
      ...
      const auto opt = base.options().dtype(common_dtype_);
      if (op.will_resize) {
    ...
    if (config.promote_inputs_to_common_dtype_ &&
        ...
        op.current_dtype != common_dtype_ && !is_cpu_scalar(i)) {
      op.exchange_tensor(
          c10::MaybeOwned<TensorBase>::owned(op.tensor().to(common_dtype_)));
      op.current_dtype = common_dtype_;
      op.target_dtype = common_dtype_;
    }
  }
...
void MusaTensorIterator::_cast_outputs() {
  for (const auto i : c10::irange(num_outputs_)) {
    auto& op = operands_[i];
    const auto& original_t = op.original_tensor();
