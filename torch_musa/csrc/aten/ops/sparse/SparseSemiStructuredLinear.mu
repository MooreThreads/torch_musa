...
#if defined(USE_ROCM) || defined(_MSC_VER)
#else
static inline void quadruple_to_cols(int q, int& c0, int& c1) {
  switch (q) {
    case 4:
      c0 = 0;
      c1 = 1;
    ...
    case 14:
      ...
      break; // 0011
    default:
      TORCH_CHECK(false, "invalid 2:4 meta nibble: ", q);
  }
}

// ---- 逆 reorder_meta：从重排后的 meta 读出逻辑 meta ----
static inline void logical_to_reordered_pos_int16(
    int64_t m,
    int64_t k_meta,
    int64_t& dest_row,
    int64_t& dest_col) {
  constexpr int64_t group = 32;
  constexpr int64_t interweave = 4;
...
// ---- decode packed+meta(2D) → dense [M,K] ----
template <typename scalar_t>
static at::Tensor decode_sparse24_2d_meta_to_dense_cpu(
    const at::Tensor& packed_cpu,
    const at::Tensor& meta_cpu,
    int64_t M,
    int64_t K) {
  TORCH_CHECK(packed_cpu.device().is_cpu());
  TORCH_CHECK(meta_cpu.device().is_cpu());
  TORCH_CHECK(packed_cpu.dim() == 2);
  TORCH_CHECK(meta_cpu.dim() == 2);
  TORCH_CHECK(packed_cpu.size(0) == M && packed_cpu.size(1) == K / 2);
  TORCH_CHECK(meta_cpu.size(0) == M && meta_cpu.size(1) == K / 16);
  TORCH_CHECK(meta_cpu.scalar_type() == at::kShort);
  TORCH_CHECK(K % 16 == 0);

  auto A = at::zeros({M, K}, packed_cpu.options());
  auto pa = packed_cpu.accessor<scalar_t, 2>();
  auto aa = A.accessor<scalar_t, 2>();

  const int64_t meta_nrows = M;
  const int64_t meta_ncols = K / 16;
  const int16_t* meta_ptr = meta_cpu.data_ptr<int16_t>();

  const int64_t nquads = K / 4;
  for (int64_t row = 0; row < M; ++row) {
    for (int64_t quad = 0; quad < nquads; ++quad) {
      const int64_t meta_col_logical = quad / 4;
      const int shift = static_cast<int>(4 * (quad % 4));

      // Step 1: row swizzle + 2x2 swizzle
      int64_t rr, rc;
      logical_to_reordered_pos_int16(row, meta_col_logical, rr, rc);

      // Step 2: ColumnMajorInterleaved<2> 物理偏移
      int64_t physical_offset = (rc / 2) * meta_nrows * 2 + rr * 2 + (rc % 2);

      const int16_t meta_word = meta_ptr[physical_offset];
      const int q = (meta_word >> shift) & 0xF;

      int c0, c1;
      quadruple_to_cols(q, c0, c1);

      const int64_t col_base = quad * 4;
      const int64_t packed_base = quad * 2;

      aa[row][col_base + c0] = pa[row][packed_base];
      aa[row][col_base + c1] = pa[row][packed_base + 1];
    }
  }
  return A;
}

// ---- fallback: decode → dense → mm ----
static inline Tensor run_cutlass_semi_structured_spmm(
  ...
  if (meta.dim() == 3) {
    ...
    return at::_sparse_semi_structured_mm(packed, meta, Bc, c10::nullopt);
  }
  TORCH_CHECK(
      meta.dim() == 2,
      "meta must be 2D [M, K/16] or 3D [M, 2, K/32], got dim=",
      meta.dim());
  TORCH_CHECK(
      meta.size(0) == m && meta.size(1) == k / 16,
      "2D meta shape mismatch: expected [",
      m,
      ", ",
      k / 16,
      "], got ",
  ...
  TORCH_CHECK(
      packed.scalar_type() == at::kHalf ||
          packed.scalar_type() == at::kBFloat16,
      "dense fallback only supports half/bfloat16, got ",
      packed.scalar_type());

  auto packed_cpu = packed.to(at::kCPU).contiguous();
  auto meta_cpu = meta.to(at::kCPU).contiguous();

  at::Tensor A_cpu;
  if (packed.scalar_type() == at::kHalf) {
    A_cpu = decode_sparse24_2d_meta_to_dense_cpu<at::Half>(
        packed_cpu, meta_cpu, m, k);
  } else {
    A_cpu = decode_sparse24_2d_meta_to_dense_cpu<c10::BFloat16>(
        packed_cpu, meta_cpu, m, k);
  }

  auto A = A_cpu.to(B.device());
  return at::mm(A, B);
}
...
Tensor two_four_sgemm(
  ...
  performance. using SmArch = mutlass::arch::Sm80; // Only CC 8.x devices are
  ...
  constexpr int AlignmentComputeEpilogue = 128 /
  ...
  mutlass::epilogue::threadblock::OutputTileThreadLayout< ThreadblockShape,
      WarpShape,
      ElementOutput,
      AlignmentOutput,
      NumEVTEpilogueStages>;

  using Accum = mutlass::epilogue::threadblock::VisitorAccFetch;

  using BiasScalar =
      mutlass::epilogue::threadblock::VisitorScalarBroadcast<ElementC>;
  using BiasTensor =
      mutlass::epilogue::threadblock::VisitorColBroadcast<
          OutputTileThreadMap,
          ElementC,
  ...
  using ApplyBias = mutlass::epilogue::threadblock::VisitorCompute<
      mutlass::plus, ElementComputeEpilogue, ElementComputeEpilogue,
      mutlass::FloatRoundStyle::round_to_nearest>;
  using EVTApplyBias = mutlass::epilogue::threadblock::Sm80EVT<
      ApplyBias,
      Accum,
      Bias>;

  using ApplyActivationNone = mutlass::epilogue::threadblock::VisitorCompute<
      mutlass::epilogue::thread::Identity,
      ElementComputeEpilogue,
      ElementComputeEpilogue,
      mutlass::FloatRoundStyle::round_to_nearest>;
  using ApplyActivationReLu = mutlass::epilogue::threadblock::VisitorCompute<
      mutlass::epilogue::thread::ReLu,
      ElementComputeEpilogue,
      ElementComputeEpilogue,
      mutlass::FloatRoundStyle::round_to_nearest>;
  using ApplyActivationSiLu = mutlass::epilogue::threadblock::VisitorCompute<
      mutlass::epilogue::thread::SiLu,
      ElementComputeEpilogue,
      ElementComputeEpilogue,
      mutlass::FloatRoundStyle::round_to_nearest>;
  using ApplyActivation =
      std::conditional_t<
          ...
          std::conditional_t<
              ...
              ApplyActivationReLu,
              ApplyActivationSiLu>>;
  using EVTApplyActivation = mutlass::epilogue::threadblock::Sm80EVT<
      ApplyActivation,
      EVTApplyBias>;

  using Output = mutlass::epilogue::threadblock::VisitorAuxStore<
      OutputTileThreadMap, ElementOutput,
  mutlass::FloatRoundStyle::round_to_nearest, cute::Stride<int64_t, cute::_1,
  int64_t>>;

  using EVTOutput = mutlass::epilogue::threadblock::Sm80EVT<
      Output,
      EVTApplyActivation>;
  ...
  } else if constexpr (std::is_same_v<ElementOutput, mutlass::half_t>) {
      tensor_d_dtype = at::kHalf;
  } else if constexpr (std::is_same_v<ElementOutput, mutlass::bfloat16_t>) {
      tensor_d_dtype = at::kBFloat16;
  } else if constexpr (std::is_same_v<ElementOutput, float>) {
      tensor_d_dtype = at::kFloat;
  } else {
      TORCH_CHECK(false, "two_four_sgemm: invalid datatype for sparse GEMM
  output ", "encountered");
  }
  ...
  */
  const auto& packed = tensor_a; // [M, K/2]
  const auto& input = tensor_b; // [..., N, K]
  const auto& bias = tensor_c;
  const int64_t M = packed.size(0);
  const int64_t K = input.size(0);

  TORCH_CHECK(input.dim() >= 2, "input must be at least 2D");
  TORCH_CHECK(packed.dim() == 2, "packed weight must be 2D");
  TORCH_CHECK(packed.size(1) * 2 == K, "packed weight/input shape mismatch");

  TORCH_CHECK(
      input.device() == packed.device(),
      "input and packed must be on same device");
  TORCH_CHECK(
      meta.device() == packed.device(),
      "meta and packed must be on same device");
  ...
  if constexpr (std::is_same_v<ElementOutput, int8_t>) {
    ...
    out_dtype = input.scalar_type();
  }

  // Step 2: run spmm using at::mm
  Tensor out_2d = run_cutlass_semi_structured_spmm(packed, meta, input, M, K);

  // Step 3: bias
  if constexpr (use_bias) {
    TORCH_CHECK(bias.defined(), "bias tensor required");
    TORCH_CHECK(bias.dim() == 1, "bias must be 1D");
    TORCH_CHECK(bias.size(0) == M, "bias size mismatch");
    out_2d = out_2d + bias.unsqueeze(1);
  }

  // Step 4: activation
  if constexpr (activation == Activation::RELU) {
    out_2d = at::relu(out_2d);
  } else if constexpr (activation == Activation::SILU) {
    out_2d = at::silu(out_2d);
  }

  return out_2d.to(out_dtype);
}
...
Tensor two_four_sgemm_dispatch_layouts(
  ...
  if constexpr (EnableRowMajorRowMajorLayouts) {
    if (tensor_a_row_major && tensor_b_row_major) {
      return two_four_sgemm<
          ...
          use_bias,
          activation>(
          tensor_a, tensor_a_stride, tensor_b, tensor_b_stride, tensor_c, meta);
    }
  }
  ...
  TORCH_CHECK(
      false,
      "two_four_sgemm_dispatch_layouts: Combination of ",
...
Tensor two_four_sgemm_dispatch_layouts_bias(
  ...
  if (tensor_c.numel() > 0) {
    return two_four_sgemm_dispatch_layouts<
        ...
        true,
        activation>(tensor_a, tensor_b, tensor_c, meta);
  } else {
    return two_four_sgemm_dispatch_layouts<
        ElementInputA,
        ElementInputB,
  ...
  if constexpr (EnableActivationNone) {
    if (activation == "none") {
      return two_four_sgemm_dispatch_layouts_bias<
          ...
          EnableColumnMajorRowMajorLayouts,
          EnableColumnMajorColumnMajorLayouts,
          Activation::NONE>(tensor_a, tensor_b, tensor_c, meta);
    }
  }
  if constexpr (EnableActivationReLU) {
    if (activation == "relu") {
      return two_four_sgemm_dispatch_layouts_bias<
          ...
          EnableColumnMajorRowMajorLayouts,
          EnableColumnMajorColumnMajorLayouts,
          Activation::RELU>(tensor_a, tensor_b, tensor_c, meta);
    }
  }
  if constexpr (EnableActivationSiLU) {
    if (activation == "silu") {
      return two_four_sgemm_dispatch_layouts_bias<
          ...
          EnableColumnMajorRowMajorLayouts,
          EnableColumnMajorColumnMajorLayouts,
          Activation::SILU>(tensor_a, tensor_b, tensor_c, meta);
    }
  }

  TORCH_CHECK(
      false,
      "two_four_sgemm_dispatch_layouts: Activation \"",
      activation,
...
// Perform linear transformation, but using corresponding MUTLASS
...
Tensor _sparse_semi_structured_linear(
  ...
  TORCH_WARN_ONCE(
      "_sparse_semi_structured_linear is deprecated and will be "
      "removed in a future PyTorch release.  Please use "
      "_sparse_semi_structured_mm/_sparse_semi_structured_addmm "
      "instead.");
#if defined(USE_ROCM) || defined(_MSC_VER)
  TORCH_CHECK(false, "_sparse_semi_structured_linear: MUTLASS not supported");
  return Tensor{};
#else
  ...
  */
  ...
  TORCH_CHECK(
      tensor_a.dtype() == at::kChar || tensor_a.dtype() == at::kHalf ||
          tensor_a.dtype() == at::kBFloat16 || tensor_a.dtype() == at::kFloat,
      "_sparse_semi_structured_linear: The weight datatype ",
      tensor_a.dtype(),
      " is not supported");
  TORCH_CHECK(
      tensor_b.dtype() == tensor_a.dtype(),
  ...
  if (tensor_c.numel() != 0) {
    TORCH_CHECK(
        tensor_c.layout() == Layout::Strided,
  ...
  TORCH_CHECK(
      tensor_a.size(1) == tensor_b.size(0) / 2,
      "_sparse_semi_structured_linear: Expected weight argument "
      "to have ",
      tensor_b.size(0) / 2,
      " columns, but got ",
  ...
  if (tensor_c.numel() != 0) {
    TORCH_CHECK(
        tensor_c.size(0) == tensor_a.size(0),
        "_sparse_semi_structured_linear: Expected bias argument "
        "to have ",
        tensor_a.size(0),
        " elements, but got ",
      ...
      AT_DISPATCH_CASE(at::ScalarType::Char, [&]() {
        ...
        if (out_dtype_opt.has_value()) {
          ...
          output = two_four_sgemm_dispatch_layouts_bias_activation<
              ...
              tensor_a, tensor_b, tensor_c, meta, activation);
        } else {
