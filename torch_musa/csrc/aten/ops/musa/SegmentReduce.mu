...
#if !(defined(_WIN32) && MUSART_VERSION == 12090)
  ...
  for (index_t j = offset_start; j < offset_end; ++j) {
    ...
    if (reduction == ReductionType::MAX) {
      initial_value =
        ...
        reduction == ReductionType::MEAN || reduction == ReductionType::SUM) {
...
__global__ void segment_reduce_backward_kernel(
  ...
  if (reduction == ReductionType::MAX || reduction == ReductionType::MIN) {
    ...
    for (int64_t j = offset_start; j < offset_end; ++j) {
      int64_t data_index = outer_idx * data_stride_axis * data_size_axis +
          j * data_stride_axis + lane_id;
      if (at::_isnan(values_data[data_index]) || values_data[data_index] == 0) {
            ...
            [&]() {
              ...
              if (output_shape.size() > 1) {
                ...
                if constexpr (std::is_same_v<scalar_t, at::BFloat16>) {
                  // BFloat16: 仍然走自定义 kernel，完全避免 CUB 的 BFloat16
                  // 实例化
                  segment_reduce_forward_kernel<scalar_t>
                      <<<num_blocks,
                         threads_per_block,
...
REGISTER_DISPATCH(
    ...
    &_segment_reduce_lengths_cuda_kernel)
REGISTER_DISPATCH(
    _segment_reduce_offsets_stub,
    &_segment_reduce_offsets_cuda_kernel)
REGISTER_DISPATCH(
