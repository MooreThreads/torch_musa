...
struct __align__(2) bf16x1 {
  __mt_bfloat16 vals[1];
};

struct __align__(4) bf16x2 {
  __mt_bfloat16 vals[2];
};

struct __align__(8) bf16x4 {
  __mt_bfloat16 vals[4];
};

struct __align__(16) bf16x8 {
  __mt_bfloat16 vals[8];
};

// bf162 vector types
struct __align__(4) bf16x2x1 {
  __mt_bfloat162 vals[1];
};

struct __align__(8) bf16x2x2 {
  __mt_bfloat162 vals[2];
};

struct __align__(16) bf16x2x4 {
  __mt_bfloat162 vals[4];
};
...
union alignas(16) PackedU32x4 {
  uint32_t vals[4];
  unsigned __int128 bits;
};

union alignas(16) PackedF32x4 {
  uint32_t u32[4];
  float4 f32;
  unsigned __int128 bits;
};

union alignas(8) PackedU32x2 {
  uint32_t vals[2];
  unsigned long long bits;
};

inline __device__ __mt_bfloat162 hfma2_scalar(
    const __mt_bfloat162 a,
    const __mt_bfloat162 b,
    const __mt_bfloat162 c) {
  return __mt_bfloat162(__hfma(a.x, b.x, c.x), __hfma(a.y, b.y, c.y));
}
...
inline __device__ bf16x2x4 convert_i4x8_to_bf16x2x4(uint32_t source) {
  ...
  auto* dst = reinterpret_cast<__mt_bfloat16*>(&result);

#pragma unroll
  for (int ii = 0; ii < kElements; ++ii) {
    const uint32_t nibble = (source >> (ii * 4)) & 0xF;
    const int32_t signed_i4 = (nibble & 0x8)
        ? (static_cast<int32_t>(nibble) - 16)
        : static_cast<int32_t>(nibble);
    dst[ii] = __float2bfloat16_rn(static_cast<float>(signed_i4));
  }
...
struct ALayout_RM {
  ...
  static __device__ void load(
    ...
    auto aPtr = reinterpret_cast<const __mt_bfloat16*>(A) + mLane * k + kLane;
...
#pragma unroll
  ...
  static __device__ void store(
    ...
    if constexpr (ReduceType == KReductionType::None) {
      ...
      auto cPtr = reinterpret_cast<__mt_bfloat16*>(C) + outRow * n + outCol;

      auto v01 = __float22bfloat162_rn(float2{out.x, out.y});
      auto v23 = __float22bfloat162_rn(float2{out.z, out.w});

      if (outRow < m) {
        *reinterpret_cast<__mt_bfloat162*>(cPtr) = v01;
      }

      // sum.z, sum.w at +8 rows from cPtr
      if (outRow + 8 < m) {
        *reinterpret_cast<__mt_bfloat162*>(cPtr + 8 * n) = v23;
      }
    }
  }
};
...
#pragma unroll
    ...
    __mt_bfloat162 qScaleAndZero[kNumQGroups];
    {
      ...
      auto qInfoPtr = reinterpret_cast<const __mt_bfloat16*>(quantizationInfo) +
          (groupStart * n + laneN) * 2;

#pragma unroll
      for (int i = 0; i < kNumQGroups; ++i) {
        qScaleAndZero[i] =
            *reinterpret_cast<const __mt_bfloat162*>(qInfoPtr + i * n * 2);
      }
    }
    ...
    //
    {
      ...
      __mt_bfloat162 qScale[kNumQGroups];
      __mt_bfloat162 qZero[kNumQGroups];
...
#pragma unroll
          for (int k = 0; k < 4; ++k) {
            v.vals[k] =
                hfma2_scalar(v.vals[k], qScale[curQGroup], qZero[curQGroup]);
          }

          // type pun, the __mt_bfloat162 value in bf16x2x4 is a struct and
...
#if defined(REAL_MUSA_VERSION) && (REAL_MUSA_VERSION >= 5100)
#pragma unroll
        for (int k = 0; k < 2; ++k) {
          PackedU32x4 fragA = {{
              a[i * kInnerKTiles + j * 2 + k].vals[0],
              a[i * kInnerKTiles + j * 2 + k].vals[1],
              a[i * kInnerKTiles + j * 2 + k].vals[2],
              a[i * kInnerKTiles + j * 2 + k].vals[3],
          }};
          PackedU32x2 fragB = {{
              b[i][(j * 2 + k) / 2].vals[((j * 2 + k) % 2) * 2 + 0],
              b[i][(j * 2 + k) / 2].vals[((j * 2 + k) % 2) * 2 + 1],
          }};
          PackedF32x4 fragD = {};
          fragD.f32 = cTmp[k];
          asm volatile(
              ".mtx\n"
              "tensor.wmma.m16n8k16.row_col.withd %0.f32, %1.b32, %2.b32, 0;"
              : "+R"(fragD.bits)
              : "R"(fragA.bits), "R"(fragB.bits));
          cTmp[k] = fragD.f32;
        }
#else
...
#pragma unroll
        for (int k = 0; k < 2; ++k) {
          ...
          wmma::fragment<
              ...
              wmma::col_major>
              wFragB;
          wmma::fragment<wmma::accumulator, 16, 8, 16, float> wFragD;
          memcpy(
              wFragA.x,
              a[i * kInnerKTiles + j * 2 + k].vals,
              sizeof(uint32_t) * 4);
          const int bFlat = j * 2 + k;
          uint32_t bVals[2] = {
              b[i][bFlat / 2].vals[(bFlat % 2) * 2 + 0],
              b[i][bFlat / 2].vals[(bFlat % 2) * 2 + 1],
          };
          memcpy(wFragB.x, bVals, sizeof(uint32_t) * 2);
          wFragD.x[0] = cTmp[k].x;
          wFragD.x[1] = cTmp[k].y;
          wFragD.x[2] = cTmp[k].z;
          wFragD.x[3] = cTmp[k].w;
          wmma::mma_sync(wFragD, wFragA, wFragB, wFragD);
          cTmp[k] = float4{wFragD.x[0], wFragD.x[1], wFragD.x[2], wFragD.x[3]};
        }
#endif
...
#pragma unroll
      for (int k = 0; k < 2; ++k) {
        ...
        PackedU32x2 fragB = {{
            b[0][(j * 2 + k) / 2].vals[((j * 2 + k) % 2) * 2 + 0],
            b[0][(j * 2 + k) / 2].vals[((j * 2 + k) % 2) * 2 + 1],
        }};
        PackedF32x4 fragD = {};
        fragD.f32 = cTmp[k];
        asm volatile(
...
#pragma unroll
      for (int k = 0; k < 2; ++k) {
        ...
        wmma::
            fragment<wmma::matrix_a, 16, 8, 16, __mt_bfloat16, wmma::row_major>
                wFragA;
        wmma::
            fragment<wmma::matrix_b, 16, 8, 16, __mt_bfloat16, wmma::col_major>
                wFragB;
        wmma::fragment<wmma::accumulator, 16, 8, 16, float> wFragD;

        memcpy(wFragA.x, a[j * 2 + k].vals, sizeof(uint32_t) * 4);

        const int bFlat = j * 2 + k;
        uint32_t bVals[2] = {
            b[0][bFlat / 2].vals[(bFlat % 2) * 2 + 0],
            b[0][bFlat / 2].vals[(bFlat % 2) * 2 + 1],
        };
        memcpy(wFragB.x, bVals, sizeof(uint32_t) * 2);
...
void launch_tinygemm_kernel(
  ...
  musaFuncAttributes funcAttr;
  C10_MUSA_CHECK(musaFuncGetAttributes(&funcAttr, func));
}
...
at::Tensor _WeightInt4packMmMusa(
    const at::Tensor& A,
    const at::Tensor& B,
    int64_t qGroupSize,
    const at::Tensor& qScaleAndZeros) {
  c10::musa::MUSAGuard g(A.device());
  musaDeviceProp prop;
  musaGetDeviceProperties(&prop, A.device().index());
  if (prop.major < 3) {
    TORCH_CHECK(
        false,
        "int4 quantized matmul is only supported on MUSA devices with compute "
        "capability 3.1 or higher");
  }
...
at::Tensor _ConvertWeight_to_Int4packMusa(
    const at::Tensor& in,
    int64_t innerKTiles) {
  c10::musa::MUSAGuard g(in.device());
  ...
  // See
  // https://github.com/meta-pytorch/gpt-fast/blob/091515ab5b06f91c0d6a3b92f9c27463f738cc9b/quantize.py#L510
  // Torch dynamo also requires the torch ops has the same output shape for each
  // device. See
  // https://github.com/pytorch/pytorch/blob/ec284d3a74ec1863685febd53687d491fd99a161/torch/_meta_registrations.py#L3263
  constexpr int32_t kNTileSizeTensor = 8;
  ...
  // Will index it correctly when dereferencing the quantized weight tensor
  // pointer.
  auto out = at::empty(
