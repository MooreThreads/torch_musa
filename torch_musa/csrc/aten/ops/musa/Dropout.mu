...
#include <ATen/musa/detail/TensorInfo.muh>
...
template <
    ...
    int VEC,
    typename mask_t>
#if __MUSA_ARCH__ >= 350 || defined(USE_ROCM)
C10_LAUNCH_BOUNDS_2(256, 4)
#endif
__global__ void
fused_dropout_kernel_vec(at::musa::detail::TensorInfo<const scalar_t, IndexType> a,
                         at::musa::detail::TensorInfo<scalar_t, IndexType> b,
                         at::musa::detail::TensorInfo<mask_t, IndexType> c,
                         IndexType totalElements, accscalar_t p,
                         PhiloxMusaState philox_args) {
  using LoadT = memory::aligned_vector<scalar_t, VEC>;
  using MaskLoadT = memory::aligned_vector<mask_t, VEC>;

  auto [seed, offset] = at::musa::philox::unpack(philox_args);
  IndexType idx = blockIdx.x * blockDim.x + threadIdx.x;
  murand_state_philox4x32_10 state;
  murand_init(seed, idx, offset, &state);

  // Helps align the total number of times murand_uniform4 is called by each thread for the same totalElements
  ...
  for (IndexType blockBase = blockIdx.x * blockDim.x * VEC;
       blockBase < totalElements;
       blockBase += gridDim.x * blockDim.x * VEC) {
   auto linearIndex = blockBase + threadIdx.x * VEC;
   if (linearIndex < totalElements) {
    ...
    //murand_uniform_double was pure evil anyway, not doing what it promises, and there's nothing for halfs, so generate float for everything
...
#endif
__global__ void
fused_dropout_kernel(musa::detail::TensorInfo<const scalar_t, IndexType> a,
                     musa::detail::TensorInfo<scalar_t, IndexType> b,
                     musa::detail::TensorInfo<mask_t, IndexType> c,
                     IndexType totalElements, accscalar_t p,
                     PhiloxMusaState philox_args) {
  auto [seed, offset] = at::musa::philox::unpack(philox_args);
  IndexType idx = blockIdx.x * blockDim.x + threadIdx.x;
...
//murand_uniform_double was pure evil anyway, not doing what it promises, and there's nothing for halfs, so generate float for everything
       float4 rand = murand_uniform4(&state);
       scalar_t src[UNROLL];
       rand.x = rand.x < p;
       rand.y = rand.y < p;
       rand.z = rand.z < p;
    ...
    // Convert `linearIndex` into an offset of `a`
               const IndexType aOffset =
                   musa::detail::IndexToOffset<const scalar_t, IndexType, ADims>::get(li, a);
    ...
    // Convert `linearIndex` into an offset of `b`
               const IndexType bOffset =
                   musa::detail::IndexToOffset<scalar_t, IndexType, BDims>::get(li, b);
...
inline void launcher(
    ...
    double p,
    const int64_t nelem,
    const PhiloxMusaState rng_engine_inputs,
    dim3 grid,
    dim3 dim_block) {
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&] {
        ...
        auto self_info =
            musa::detail::getTensorInfo<const scalar_t, index_type>(self);
        auto ret_info =
            musa::detail::getTensorInfo<scalar_t, index_type>(ret);
        auto mask_info =
            musa::detail::getTensorInfo<mask_t, index_type>(mask);
          ...
          switch (vec_size) {
            case 16:
              fused_dropout_kernel_vec<
                  ...
                  1,
                  16>
                  <<<grid, dim_block, 0, at::musa::getCurrentMUSAStream()>>>(
                      self_info,
                      ret_info,
                      mask_info,
                      nelem,
...
dropout_cuda(MUSAGeneratorImpl* gen, const Tensor& self, double p){
  ...
  unsigned int blocks_per_sm = at::musa::getCurrentDeviceProperties()->maxThreadsPerMultiProcessor/block_size;
  dim3 dim_block(block_size);
  dim3 grid((nelem + block_size -1)/block_size);
  grid.x = std::min((unsigned int)at::musa::getCurrentDeviceProperties()->multiProcessorCount * blocks_per_sm, grid.x);
//number of times random will be generated per thread, to offset philox counter in thc random state
  ...
  if (musa::detail::canUse32BitIndexMath(self)){
...
native_dropout_cuda(const Tensor& self, double p, std::optional<bool> train){
  ...
  auto gen = get_generator_or_default<MUSAGeneratorImpl>(std::nullopt, musa::detail::getDefaultMUSAGenerator());
  double p1m = 1. - p;
...
fused_dropout_cuda(const Tensor& self, double p, std::optional<Generator> gen_){
  auto gen = get_generator_or_default<MUSAGeneratorImpl>(gen_, musa::detail::getDefaultMUSAGenerator());
