...
struct HermitianSymmetryOffsetCalculator {
  ...
  dim_type dims;
  at::musa::detail::IntDivider<index_t> sizes_[MAX_DIMS];
  ...
  HermitianSymmetryOffsetCalculator(
    ...
    using at::musa::detail::IntDivider;
...
// out[:] = conj(in[:]) where in and out ordering is generalized by offset
// calculators
template <typename scalar_t, typename inp_calc_t, typename out_calc_t>
C10_LAUNCH_BOUNDS_1(musa::detail::CUDA_NUM_THREADS)
...
void _fft_fill_with_conjugate_symmetry_musa_(
    ScalarType dtype,
  ...
  AT_DISPATCH_COMPLEX_TYPES_AND(
      kComplexHalf, dtype, "_fft_fill_with_conjugate_symmetry", [&] {
        using namespace musa::detail;
...
REGISTER_DISPATCH(
    ...
    &_fft_fill_with_conjugate_symmetry_musa_);
