...
template <typename T>
MUSA_DEVICE_NOINLINE T ni_log(T a) {
  return ::log(a);
}
template <typename T>
MUSA_DEVICE_NOINLINE T ni_log10(T a) {
  return ::log10(a);
}
template <typename T>
MUSA_DEVICE_NOINLINE T ni_log2(T a) {
  return ::log2(a);
}
template <typename T>
MUSA_DEVICE_NOINLINE T ni_log1p(T a) {
  return ::log1p(a);
}
...
void log_kernel_cuda(TensorIteratorBase& iter) {
  ...
  if (at::isComplexType(common_dtype)) {
    ...
    AT_DISPATCH_COMPLEX_TYPES_AND(
        kComplexHalf, common_dtype, "log_cuda", [&]() {
          jitted_gpu_kernel<
              /*name=*/log_name,
...
#else
    AT_DISPATCH_COMPLEX_TYPES_AND(
        kComplexHalf, iter.common_dtype(), "log_cuda", [&]() {
          gpu_kernel(iter, [] GPU_LAMBDA(scalar_t a) -> scalar_t {
            using opmath_t = at::opmath_type<scalar_t>;
            return ni_log<opmath_t>(static_cast<opmath_t>(a));
          });
        });
#endif
  } else {
