...
template <typename T>
MUSA_DEVICE_NOINLINE T ni_exp(T a) {
  return std::exp(a);
}
template <typename T>
MUSA_DEVICE_NOINLINE T ni_expm1(T a) {
  return ::expm1(a);
}
template <typename T>
MUSA_DEVICE_NOINLINE T ni_sqrt(T a) {
  return ::sqrt(a);
}
...
#if AT_USE_JITERATOR()
    static const auto exp_string =
        jiterator_stringify(template <typename T> T exp_kernel(T x) {
          return std::exp(x);
        }); // exp_string
    ...
    AT_DISPATCH_COMPLEX_TYPES_AND(
        kComplexHalf, common_dtype, "exp_cuda", [&]() {
          gpu_kernel(iter, [] GPU_LAMBDA(scalar_t a) -> scalar_t {
            ...
            return ni_exp<opmath_t>(static_cast<opmath_t>(a));
          });
        });
#endif
  } else {
...
C10_HOST_DEVICE static inline __attribute__((noinline)) scalar_t rsqrt_wrapper(
    scalar_t v) {
  return ::rsqrt(v);
}

template <typename T>
C10_HOST_DEVICE static inline __attribute__((noinline)) c10::complex<T>
rsqrt_wrapper(c10::complex<T> v) {
...
void rsqrt_kernel_cuda(TensorIteratorBase& iter) {
  ...
  if (at::isComplexType(common_dtype)) {
#if AT_USE_JITERATOR()
    static const auto rsqrt_string =
        jiterator_stringify(template <typename T> T rsqrt_kernel(T x) {
          const T one = T{1};
          return one / std::sqrt(x);
        }); // rsqrt_string
    AT_DISPATCH_COMPLEX_TYPES_AND(
        kComplexHalf, common_dtype, "rsqrt_cuda", [&]() {
          jitted_gpu_kernel<
              /*name=*/rsqrt_name,
              /*return_dtype=*/scalar_t,
              /*common_dtype=*/scalar_t,
...
void sqrt_kernel_cuda(TensorIteratorBase& iter) {
  ...
  if (at::isComplexType(common_dtype)) {
#if AT_USE_JITERATOR()
    static const auto sqrt_string =
        jiterator_stringify(template <typename T> T sqrt_kernel(T x) {
          return std::sqrt(x);
        }); // sqrt_string
    AT_DISPATCH_COMPLEX_TYPES_AND(
        kComplexHalf, common_dtype, "sqrt_cuda", [&]() {
          jitted_gpu_kernel<
              /*name=*/sqrt_name,
              /*return_dtype=*/scalar_t,
              /*common_dtype=*/scalar_t,
    ...
    AT_DISPATCH_COMPLEX_TYPES_AND(
        kComplexHalf, common_dtype, "sqrt_cuda", [&]() {
          gpu_kernel(iter, [] GPU_LAMBDA(scalar_t a) -> scalar_t {
            using opmath_t = at::opmath_type<scalar_t>;
            return ni_sqrt<opmath_t>(static_cast<opmath_t>(a));
          });
        });
#endif
  } else {
...
template <typename scalar_t>
C10_HOST_DEVICE static inline scalar_t _nan_to_num_replace(
    scalar_t a,
...
void nan_to_num_kernel_cuda(
  ...
  if (isComplexType(iter.dtype())) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        kHalf, kBFloat16, iter.dtype(), "nan_to_num_cuda", [&]() {
