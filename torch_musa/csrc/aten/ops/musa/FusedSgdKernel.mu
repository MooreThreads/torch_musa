...
C10_DEVICE __forceinline__ void sgd_math(
    scalar_t r_args[depth][native::kILP],
...
#pragma unroll
  for (int ii = 0; ii < native::kILP; ii++) {
...
struct FusedSgdMathFunctor {
  ...
  C10_DEVICE __forceinline__ void operator()(
      ...
      native::TensorListMetadata<depth>& tl,
      const double weight_decay,
      const double momentum,
      const float* lr_ptr,
      const double lr,
    ...
    const auto use_faster_load_store = (n % native::kILP == 0) &&
        (chunk_size % native::kILP == 0) && all_aligned;
    if (use_faster_load_store) {
      for (auto i_start = threadIdx.x;
           i_start * native::kILP < n && i_start * native::kILP < chunk_size;
           i_start += blockDim.x) {
#pragma unroll
        for (auto i = 0; i < depth; i++) {
          native::load_store(r_args[i], args[i], 0, i_start);
        }
        sgd_math<scalar_t, depth>(
            ...
            is_first_step,
            grad_scale_ptr);
        native::load_store(args[0], r_args[0], i_start, 0);
        if (grad_scale_ptr) {
          native::load_store(args[1], r_args[1], i_start, 0);
        }
        if (depth > 2) {
          native::load_store(args[2], r_args[2], i_start, 0);
        }
      }
    } else {
      for (auto i_start = 0; i_start < n && i_start < chunk_size;
           i_start += blockDim.x * native::kILP) {
        native::load_args<depth>(r_args, args, i_start, chunk_size, n);
        sgd_math<scalar_t, depth>(
            r_args,
            weight_decay,
            momentum,
            ...
            is_first_step,
            grad_scale_ptr);
        native::store_args(args[0], r_args[0], i_start, chunk_size, n);
        if (grad_scale_ptr) {
          native::store_args(args[1], r_args[1], i_start, chunk_size, n);
        }
        if (depth > 2) {
          native::store_args(args[2], r_args[2], i_start, chunk_size, n);
        }
      }
    }
  }
...
void _fused_sgd_with_momentum_kernel_musa_(
  ...
  TORCH_MUSA_CHECK_GT(momentum, 0);
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      "fused_sgd_with_momentum_kernel_musa",
      [&]() {
        native::multi_tensor_apply<3>(
            tensor_lists,
...
void _fused_sgd_with_momentum_kernel_musa_(
    ...
    const std::optional<at::Tensor>& found_inf) {
  if (lr.is_cpu()) {
    _fused_sgd_with_momentum_kernel_musa_(
        params,
        grads,
...
void FusedSgdKernelMusa(
  ...
  TORCH_MUSA_CHECK_EQ(momentum, 0);
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      kBFloat16,
      params[0].scalar_type(),
      "fused_sgd_kernel_musa",
      [&]() {
        native::multi_tensor_apply<2>(
            tensor_lists,
