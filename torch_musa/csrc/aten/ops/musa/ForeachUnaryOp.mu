...
// makes the functor
// use force noinline inplace original impl (operator() delegates to noinline
// ni_fn_*)
#define STD_FUNCTOR(op_name, functor_name)           \
  template <typename T>                              \
  MUSA_DEVICE_NOINLINE T ni_fn_##functor_name(T t) { \
    return std::op_name(t);                          \
  }                                                  \
  template <typename T>                              \
  struct functor_name {                              \
    __device__ T operator()(T t) const {             \
      return ni_fn_##functor_name<T>(t);             \
    }                                                \
  };
...
//
template <typename T>
MUSA_DEVICE_NOINLINE T ni_sigmoid_fe(T t) {
  return T(1) / (T(1) + std::exp(-t));
}
template <typename T>
struct Sigmoid {
  __device__ T operator()(T t) const {
    return ni_sigmoid_fe<T>(t);
  }
};
...
struct Rsqrt {
  C10_DEVICE T operator()(T t) const {
    return c10::musa::compat::rsqrt(t);
  }
};
