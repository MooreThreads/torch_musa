...
template <typename T>
MUSA_DEVICE_NOINLINE T ni_logaddexp_real(T a, T b) {
  if (::isinf(a) && a == b)
    return a;
...
template <typename T>
MUSA_DEVICE_NOINLINE T ni_logaddexp2_real(T a, T b, T inv_log_2) {
  if (::isinf(a) && a == b)
    return a;
  const auto m = ::max(a, b);
...
__host__ __device__ __attribute__((noinline)) c10::complex<scalar_t>
_logaddexp_minmax(
...
template <typename scalar_t>
__host__ __device__ __attribute__((noinline)) c10::complex<scalar_t>
_fast_build_exp(const c10::complex<scalar_t>& x) {
...
template <typename scalar_t>
__host__ __device__ __attribute__((noinline)) c10::complex<scalar_t>
_fast_build_exp_inf(const c10::complex<scalar_t>& x) {
  // complex exponential function, but implemented manually to get fast
...
const auto logaddexp_complex_string = jiterator_stringify(
    ...
    // separated _logaddexp_minmax into 2 different functions for
    // jiterator_string
    template <typename T>
    std::complex<T> logaddexp_min(
    ...
    complex_t logaddexp_complex(complex_t x, complex_t y) {
      ...
      if (isnan(min_real) || isnan(min_val.imag())) {
        ...
        if (!isfinite(minmax.real())) {
          exp_minmax = (minmax.real() < T(0)) ? complex_t(0, 0)
                                              : fast_build_exp_inf<T>(minmax);
        } else {
...
void logaddexp_kernel_cuda(TensorIteratorBase& iter) {
  if (at::isComplexType(iter.dtype())) {
    ...
    AT_DISPATCH_COMPLEX_TYPES_AND(
        at::ScalarType::ComplexHalf, iter.dtype(), "logaddexp_cuda", [&]() {
          jitted_gpu_kernel<
...
void logaddexp2_kernel_cuda(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&]() {
        ...
        gpu_kernel(
            iter, [inv_log_2] GPU_LAMBDA(scalar_t a_, scalar_t b_) -> scalar_t {
              const auto a = static_cast<opmath_t>(a_);
              const auto b = static_cast<opmath_t>(b_);
              return ni_logaddexp2_real(a, b, inv_log_2);
            });
      });
}
