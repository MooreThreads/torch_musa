...
// There are no significant changes to the original CUDA kernel design
// principles, but with several changes below:
// 1. `aligned_array` replaces the fixed `stream_load128`, which makes tuning
//   the kernel much easier.
// 2. The less efficient division operation in the `chunk_cat` kernel has also
// been removed.
// 3. Make each thread process OUTPUT_BYTES_PER_THREAD * iter_factor bytes to
// regulate block size in chunk_cat kernel.
...
static constexpr int64_t BLOCK_SIZE = 512;
static constexpr int64_t BYTES_PER_THREAD = 16;
static constexpr int64_t BYTES_PER_BLOCK = BYTES_PER_THREAD * BLOCK_SIZE;

// these two constexpr will be used by chunk_cat kernel
static constexpr int64_t OUTPUT_BYTES_PER_THREAD = 16;
static constexpr int64_t OUTPUT_BYTES_PER_BLOCK =
    OUTPUT_BYTES_PER_THREAD * BLOCK_SIZE;
...
struct __align__(Alignment) aligned_array {
  T data[N];
};
...
static __device__ __inline__ void copy_chunk(
  ...
  using access_t = aligned_array<char, detail::BYTES_PER_THREAD>;

  get_aligned_region(
      dst, chunk_size, detail::BYTES_PER_THREAD, align_off, aligned_size);
  // dst, chunk_size, 1, align_off, aligned_size);
  ...
  for (int64_t off = align_off + thread_idx * detail::BYTES_PER_THREAD;
    ...
    access_t val = *(reinterpret_cast<const access_t*>(src + off));
    *reinterpret_cast<access_t*>(dst + off) = val;
  }
...
__device__ __inline__ void copy_chunk_with_pad(
    dst_t* dst_ptr,
    src_t* src_ptr,
    int64_t max_chunk_numel,
    int64_t actual_chunk_numel,
    int64_t thread_idx,
    int64_t num_threads,
  ...
  int64_t max_chunk_size = max_chunk_numel * dst_elem_size;
  int64_t actual_chunk_size = actual_chunk_numel * src_elem_size;

  // Fast path when the number of threads is larger than the number of bytes
  // to be copied (i.e., max_chunk_size). In this case, each thread only copies
  // 1 byte. For 0 <= thread_idx < actual_chunk_size, the thread copyies data
  // from `src`. For actual_chunk_size <= thread_idx < max_chunk_size, the
  // thread set the val=0 for padding.
  ...
  // Not explicitly handling alignment even more performant ?
  using access_t = aligned_array<char, OUTPUT_BYTES_PER_THREAD>;
  int64_t align_off, aligned_size;
  get_aligned_region(
      // dst, actual_chunk_size, OUTPUT_BYTES_PER_THREAD, align_off,
      // aligned_size);
      dst,
      actual_chunk_size,
      1,
  ...
  // 1. copy datas at aligned region
  for (int64_t i = align_off + thread_idx * OUTPUT_BYTES_PER_THREAD;
       i < align_end;
       i += num_threads * OUTPUT_BYTES_PER_THREAD) {
    // Oppurtunistically vectorize reads/writes
    access_t val = *(reinterpret_cast<const access_t*>(src + i));
    *reinterpret_cast<access_t*>(dst + i) = val;
  }

  // Copy data for the first part of dst array, [dst, dst + align_off)
  // Check `thread_idx < max_chunk_size` for the eage case that max_chunk_size <
  // align_off the number of align_off should be less than
  // OUTPUT_BYTES_PER_THREAD
  ...
  // Copy data for the third part of dst array
  // [dst + align_end, dst + max_chunk_size).
...
// chunk_cat_musa adopts a "jagged grid" strategy, inspired by NOTE [CUDA fast
// path for split_with_sizes_copy.out]. In addition, chunk_cat_musa supports
// padding via copy_chunk_with_pad when src chunk size is less than dst chunk
// size.
template <typename dst_t, typename src_t>
static __global__ void chunk_cat_musa_kernel(
    src_t** src,
    dst_t* dst,
    ...
    int64_t* pad_tensor_chunk_numels,
  ...
  // the pad_tensor_chunk_sizes[tensor_idx] is # of bytes regard on
  // tensor_idx-th tensor
  //
  int64_t pad_tensor_src_chunk_size =
      pad_tensor_chunk_numels[tensor_idx] * src_elem_size;
  char* src_addr = reinterpret_cast<char**>(src)[tensor_idx] +
      slice_idx * actual_tensor_numels[tensor_idx] * src_elem_size +
      chunk_idx * (int64_t)pad_tensor_src_chunk_size;
  ...
  // Compute the actual number of elements to copy from src.
  const int64_t actual_copy_numel = std::min(
      pad_tensor_chunk_numels[tensor_idx],
      std::max(
          (int64_t)0,
          actual_tensor_numels[tensor_idx] -
              chunk_idx * (int64_t)pad_tensor_chunk_numels[tensor_idx]));
...
get_chunk_cat_metadata(
  ...
  std::vector<int64_t> pad_tensor_chunk_numels;
  ...
  for (const auto i : c10::irange(num_tensors)) {
    ...
    const int64_t pad_tensor_chunk_numel =
        pad_size_along_dim * trailing_numel / num_chunks;
    const int64_t pad_tensor_chunk_size =
        pad_tensor_chunk_numel * dst_elem_size;
    pad_tensor_chunk_numels.push_back(pad_tensor_chunk_numel);
    chunk_size += pad_tensor_chunk_size;
    // Number of blocks required to process this tensor chunk.
    // Make each thread process OUTPUT_BYTES_PER_THREAD * iter_factor bytes to
    // regulate block size, though iter_factor is hard coded to 8.
    const int64_t num_blocks = detail::div_up(
        pad_tensor_chunk_size, detail::OUTPUT_BYTES_PER_BLOCK * 8);
    ...
    // actual_tensor_sizes.push_back(sizes[dim] * trailing_numel *
    // src_elem_size);
    actual_tensor_numels.push_back(sizes[dim] * trailing_numel);
  }
  ...
  return std::make_tuple(
      ...
      pad_tensor_chunk_numels,
      num_blocks_per_tensor_chunk);
}

// See [CUDA kernel for chunk_cat]
template <typename dst_t, typename src_t>
void _chunk_cat_out_musa_contiguous(
    TensorList tensors,
    int64_t dim,
    int64_t num_chunks,
    Tensor& out,
...
void split_with_sizes_copy_out_musa_contiguous_no_cast(
  ...
  // latency.
  const auto num_sms =
      at::musa::getCurrentDeviceProperties()->multiProcessorCount;
...
void SplitWithSizesCopyOutMUSA(
    const Tensor& self,
    IntArrayRef split_sizes,
    int64_t dim,
    TensorList out) {
  const bool is_capturing = at::musa::currentStreamCaptureStatusMayInitCtx() !=
  ...
  if (!is_capturing && contiguous_no_cast) {
    ...
    for (const auto i : c10::irange(split_sizes.size())) {
      ...
      if (native::resize_output_check(out[i], out_shape)) {
    ...
    split_with_sizes_copy_out_musa_contiguous_no_cast(
...
Tensor ChunkCatMUSA(TensorList tensors, int64_t dim, int64_t num_chunks) {
  ...
  if (detail::all_contiguous(tensors)) {
    ...
    detail::_chunk_cat_out_musa_contiguous<char, char>(
...
Tensor& ChunkCatMUSAOut(
  ...
  TORCH_CHECK(
      ...
      "ChunkCatMUSAOut: mismatch between input and out tensor devices");
  ...
  if (both_input_output_contiguous &&
      (tensors[0].dtype() == at::ScalarType::BFloat16 ||
       tensors[0].dtype() == at::ScalarType::Half) &&
      (out.dtype() == at::ScalarType::Float)) {
    // _chunk_cat_out_musa_contiguous should also support other types, thanks to
    // static_cast_with_inter_type. Here, we dispatch to BFloat16/Half in and
    // float32 out since it is the only known use case.
#define AT_DISPATCH_CASE_BFLOAT16_AND_HALF_IN(...)        \
  AT_DISPATCH_CASE(at::ScalarType::BFloat16, __VA_ARGS__) \
  AT_DISPATCH_CASE(at::ScalarType::Half, __VA_ARGS__)

    AT_DISPATCH_SWITCH(
        tensors[0].scalar_type(),
        "_chunk_cat_out_musa_contiguous",
        AT_DISPATCH_CASE_BFLOAT16_AND_HALF_IN([&] {
          detail::_chunk_cat_out_musa_contiguous<float, scalar_t>(
              tensors,
              dim,
              num_chunks,
              out,
...
#undef AT_DISPATCH_CASE_BFLOAT16_AND_HALF_IN
  } else if (
