...
template <typename T>
MUSA_DEVICE_NOINLINE T ni_chebyshev_polynomial_u_forward(T x, T n) {
  return chebyshev_polynomial_u_forward<T, true>(x, n);
}
namespace {
...
void chebyshev_polynomial_u_kernel_cuda(TensorIteratorBase& iterator) {
  ...
  AT_DISPATCH_FLOATING_TYPES(
      iterator.common_dtype(), "chebyshev_polynomial_u_cuda", [&]() {
        opmath_jitted_gpu_kernel_with_scalars<
            chebyshev_polynomial_u_name,
            scalar_t,
            scalar_t>(iterator, chebyshev_polynomial_u_string);
      });
#else
  AT_DISPATCH_FLOATING_TYPES(
      iterator.common_dtype(), "chebyshev_polynomial_u_cuda", [&]() {
        gpu_kernel_with_scalars(
            iterator, [] GPU_LAMBDA(scalar_t x, scalar_t n) -> scalar_t {
              return ni_chebyshev_polynomial_u_forward<scalar_t>(x, n);
            });
      });
#endif
