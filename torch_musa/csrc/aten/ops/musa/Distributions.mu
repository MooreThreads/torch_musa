...
#include <ATen/native/musa/DistributionTemplates.h>
#include <ATen/native/musa/Distributions.h>
#include <ATen/musa/MUSAApplyUtils.muh>
#include "torch_musa/csrc/aten/musa/MUSAGeneratorImpl.h"

#include <murand.h>
#include <murand_kernel.h>
#include <murand_philox4x32_10.h>
#include <functional>
#include <utility>
...
template <typename scalar_t, typename accscalar_t, typename U, typename N>
MUSA_DEVICE_NOINLINE auto ni_sample_gamma(
    scalar_t alpha,
    BaseSampler<accscalar_t, U>& su,
    BaseSampler<accscalar_t, N>& sn) {
  return sample_gamma<scalar_t, accscalar_t, U, N>(alpha, su, sn);
}
template <typename scalar_t, typename accscalar_t, typename U>
MUSA_DEVICE_NOINLINE auto ni_sample_binomial(
    scalar_t count,
    scalar_t prob,
    BaseSampler<accscalar_t, U>& su) {
  return sample_binomial<scalar_t, accscalar_t, U>(count, prob, su);
}
template <typename T>
MUSA_DEVICE_NOINLINE auto ni_murand_poisson(
    murand_state_philox4x32_10* state,
    T lambda) {
  return murand_poisson(state, lambda);
}
...
void poisson_cuda_kernel(
  ...
  auto functor = [philox_args] __device__(
    ...
    murand_init(
        ...
        std::get<1>(seeds),
        &state);
    ret_val = static_cast<scalar_t>(ni_murand_poisson(&state, lambda));
  };
  at::musa::CUDA_tensor_apply2<
      scalar_t,
      scalar_t,
      decltype(functor),
...
struct curand_uniform_wrapper {
  murand_state_philox4x32_10& state;
  __device__ curand_uniform_wrapper(murand_state_philox4x32_10& state)
      : state(state) {}
  __device__ float operator()() {
    uint32_t val = murand(&state); // need just bits
...
void binomial_cuda_kernel(
  ...
  at::native::distribution_binary_kernel(
      iter,
      philox_args,
      [] GPU_LAMBDA(
          murand_state_philox4x32_10 & state, scalar_t count, scalar_t prob) {
        auto uniform_lambda = curand_uniform_wrapper(state);
        BaseSampler<accscalar_t, decltype(uniform_lambda)> standard_uniform(
            uniform_lambda);
        auto sample =
            ni_sample_binomial<scalar_t, accscalar_t, decltype(uniform_lambda)>(
...
MUSA_DEVICE_NOINLINE auto ni_sgg(scalar_t a, scalar_t b) {
  return standard_gamma_grad_one<scalar_t, accscalar_t>(a, b);
}
template <typename scalar_t, typename accscalar_t>
MUSA_DEVICE_NOINLINE auto ni_dgo(scalar_t x, scalar_t a, scalar_t t) {
  return dirichlet_grad_one<scalar_t, accscalar_t>(x, a, t);
}
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&] {
        at::native::gpu_kernel(
            iter, [] GPU_LAMBDA(scalar_t gamma, scalar_t gamma_sum) {
...
void launch_poisson_cuda_kernel(
    ...
    MUSAGeneratorImpl* gen) {
  PhiloxMusaState rng_engine_inputs;
  {
    // See Note [Acquire lock when using random generators]
    std::lock_guard<std::mutex> lock(gen->mutex_);
    rng_engine_inputs = gen->philox_musa_state(20);
  }
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::ScalarType::Half,
      at::ScalarType::BFloat16,
...
void launch_binomial_cuda_kernel(
  ...
  PhiloxMusaState rng_engine_inputs;
  {
    // See Note [Acquire lock when using random generators]
    std::lock_guard<std::mutex> lock(gen->mutex_);
    rng_engine_inputs = gen->philox_musa_state(42);
  }
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::ScalarType::Half,
      at::ScalarType::BFloat16,
...
void launch_gamma_kernel(
  ...
  PhiloxMusaState rng_engine_inputs;
  {
    // See Note [Acquire lock when using random generators]
    std::lock_guard<std::mutex> lock(gen->mutex_);
    rng_engine_inputs = gen->philox_musa_state(10);
  }
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::ScalarType::Half,
      at::ScalarType::BFloat16,
...
void launch_standard_gamma_grad_kernel(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      at::ScalarType::BFloat16,
      iter.input_dtype(),
      "_standard_gamma_grad_cuda",
      [&] {
        using accscalar_t = at::acc_type<scalar_t, true>;
        gpu_kernel(iter, [] GPU_LAMBDA(scalar_t self_val, scalar_t output_val) {
          return ni_sgg<scalar_t, accscalar_t>(self_val, output_val);
        });
      });
}
...
void launch_dirichlet_grad_kernel(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_TYPES(iter.input_dtype(), "_dirichlet_grad_cuda", [&] {
    ...
    at::native::gpu_kernel(
        iter,
        [] GPU_LAMBDA(scalar_t x_val, scalar_t alpha_val, scalar_t total_val)
            -> scalar_t {
          return ni_dgo<scalar_t, accscalar_t>(x_val, alpha_val, total_val);
        });
  });
}
