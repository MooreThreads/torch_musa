...
// amp_update_scale_musa_kernel is launched with a single thread to compute the
// new scale. The scale factor is maintained and updated on the GPU to avoid
// synchronization.
__global__ void amp_update_scale_musa_kernel(
    float* current_scale,
    int* growth_tracker,
    float* found_inf,
    float growth_factor,
...
// growth_tracker:  A one-element torch.musa.IntTensor containing the number of
// recent consecutive unskipped steps. found_inf:  A one-element musa float
...
Tensor& AmpUpdateScaleMusa(
  ...
  TORCH_CHECK(
      growth_tracker.is_privateuseone(),
      "growth_tracker must be a MUSA tensor.");
  TORCH_CHECK(
      current_scale.is_privateuseone(), "current_scale must be a MUSA tensor.");
