...
inline bool getCatGrid(ptrdiff_t nTensors, dim3& grid) {
  const int numSM = at::musa::getCurrentDeviceProperties()->multiProcessorCount;
...
template <typename T>
inline std::tuple<dim3, dim3> getCatGridRocm(
...
template <typename T>
inline std::tuple<dim3, dim3> getCatGridContig(
    unsigned int max_elements_per_tensor,
    ptrdiff_t nTensors) {
...
__global__ void CatArrayBatchedCopy(
  ...
  TensorSizeStride<IndexType, CAT_ARRAY_MAX_INPUT_DIMS> ins = stride_size > 1
      ? inputs.tensorStride[blockIdx.y]
...
void parallel_cat(
  ...
  for (unsigned i = 0; i < inputs.size(); i += batch_size) {
    ...
    if (isContig && sizeof(scalar_t) > 2) {
      std::tie(catGrid, applyBlock) =
          getCatGridContig<scalar_t>(max_elements_per_tensor, batchCounter);
    } else {
...
#define HANDLE_CASE(DIMS)                                                      \
  if (isContig && isAligned && sizeof(scalar_t) >= 4 &&                        \
    ...
    CatArrayBatchedCopy_aligned16_contig<                                      \
        ...
        DIMS,                                                                  \
        batch_size,                                                            \
        stride_size><<<catGrid, applyBlock, 0, stream.stream()>>>(             \
        data,                                                                  \
        catMetaData,                                                           \
        outputParam,                                                           \
        dimension,                                                             \
...
void cat_complex_out_musa(
  ...
  const bool all32BitIndexable = std::all_of(
      materialized.begin(), materialized.end(), [](const Tensor& t) {
        return at::musa::detail::canUse32BitIndexMath(t);
      });
  ...
  // to musa kernel through constant memory. Therefore, we could pass more
  // inputs to musa threads. For non-contiguous, we reduce the number of inputs
  // passed to musa kernel due to the limitation of constant memory.

  if (materialized.size() > 1 && result.dim() <= CAT_ARRAY_MAX_INPUT_DIMS &&
      at::musa::detail::canUse32BitIndexMath(result) && all_contiguous &&
    ...
    AT_DISPATCH_V2(
      ...
      at::musa::detail::canUse32BitIndexMath(result) &&
