/*
This kernel is borrowed from pytorch to adapt musa devices cause the warp
size on musa is 128 currently, so there would have some errors during runtime
*/
#define TORCH_ASSERT_NO_OPERATORS
#include <ATen/Dispatch.h>
#include <ATen/musa/ThrustAllocator.h>
#include <ATen/native/NonEmptyUtils.h>
#include <ATen/native/musa/TensorModeKernel.h>
#include <c10/core/DeviceArray.h>
#include <ATen/musa/detail/IndexUtils.muh>
#include <ATen/native/musa/TensorModeKernel.muh>
...
struct ModeImpl {
  std::tuple<scalar_t, int64_t> operator()(
      scalar_t* iter_begin,
      scalar_t* iter_end) {
    at::musa::ThrustAllocator thrust_allocator;
    auto stream = at::musa::getCurrentMUSAStream();
    auto policy = thrust::musa::par(thrust_allocator).on(stream);

    const auto n_element = iter_end - iter_begin;
    auto musa_allocator = at::musa::getMUSADeviceAllocator();
    auto sort_buffer = c10::DeviceArray<int64_t>(*musa_allocator, n_element);
    ...
    // Count # of unique elements via an inner product between adjacent
    // elements. Add 1 if two neighboring element are not equal.
    int unique = 1 +
    ...
    auto keys = c10::DeviceArray<scalar_t>(*musa_allocator, unique);
    auto counts = c10::DeviceArray<int64_t>(*musa_allocator, unique);
...
struct ModeImpl<bool> {
  std::tuple<bool, int64_t> operator()(const bool* first, const bool* last) {
    at::musa::ThrustAllocator thrust_allocator;
    auto stream = at::musa::getCurrentMUSAStream();
    auto policy = thrust::musa::par(thrust_allocator).on(stream);
    ...
    const auto num_true = thrust::count_if(
        policy, first_bytes, last_bytes, [] GPU_LAMBDA(uint8_t x) {
    ...
    const auto position_iter =
        thrust::find_if(policy, first_bytes, last_bytes, EqualsMode{mode});
...
void calculate_mode(
  ...
  AT_MUSA_CHECK(musaMemcpyAsync(
      values_data, &mode, sizeof(scalar_t), musaMemcpyHostToDevice, stream));
  // memcpy_and_sync will synchronize results
  at::musa::memcpy_and_sync(
      indices_data, &index, sizeof(int64_t), musaMemcpyHostToDevice, stream);
}
...
void handle_fused_mode(
    dim3 grid,
    const TensorBase& self,
    musa::detail::TensorInfo<scalar_t, unsigned int>& ti_values,
    musa::detail::TensorInfo<int64_t, unsigned int>& ti_indices,
    int64_t slice_size,
    int64_t slices) {
...
void fused_mode(
  ...
  auto ti_values = musa::detail::getTensorInfo<scalar_t, unsigned int>(values);
  auto ti_indices = musa::detail::getTensorInfo<int64_t, unsigned int>(indices);
...
void launch_fused_mode_kernel(
  ...
  AT_DISPATCH_ALL_TYPES_AND3(
      kBool, kBFloat16, kHalf, self.scalar_type(), "musa_mode", [&] {
