  ...
  SegmentRange map(SegmentRange range) {
    ...
    for (auto i : c10::irange(begin, end)) {
      ...
      static const bool enable_ipc_handles =
          c10::utils::check_env("TORCH_MUSA_EXPANDABLE_SEGMENTS_IPC")
              .value_or(true);
...
#define SYS_pidfd_getfd 438
    ...
    if (MUSAAllocatorConfig::expandable_segments_handle_type() !=
      ...
      for (auto i : c10::irange(header.num_handles)) {
        ...
        MUmemFabricHandle fabric_handle;
        buf.read(
            reinterpret_cast<char*>(&fabric_handle), sizeof(MUmemFabricHandle));
        MUmemGenericAllocationHandle handle = 0;
        C10_MUSA_DRIVER_CHECK(DriverAPI::get()->muMemImportFromShareableHandle_(
            &handle,
            // NOLINTNEXTLINE(performance-no-int-to-ptr)
  ...
  void unmapHandles(size_t begin, size_t end) {
    ...
    for (auto i : c10::irange(begin, end)) {
      ...
      if (h.shareable_handle &&
          std::holds_alternative<int>(*h.shareable_handle)) {
...
struct PrivatePool {
  ...
  std::shared_ptr<MUSAAllocator> allocator_;
...
static std::string reportProcessMemoryInfo(const musaDeviceProp& prop) {
  return "";
}
...
class DeviceCachingAllocator {
 private:
  ...
  struct GraphReuseContext {
    ska::flat_hash_map<musaStream_t, ska::flat_hash_set<musaGraphNode_t>>
        visited;
  };
 ...
 public:
  ...
  explicit DeviceCachingAllocator(c10::DeviceIndex id)
    ...
    C10_MUSA_CHECK(musaGetDeviceProperties(&device_prop, id));
    setMemoryFraction(MUSAAllocatorConfig::per_process_memory_fraction());
  ...
  Block* malloc(size_t orig_size, musaStream_t stream) {
    ...
    if (!block_found) {
      ...
      c10::reportOutOfMemoryToProfiler(
          ...
          stats.reserved_bytes[static_cast<int64_t>(StatType::AGGREGATE)]
              .current,
          c10::Device(at::musa::kMUSA, device_id));
  ...
  struct CaptureInfo {
    ...
    const musaGraphNode_t* terminals{nullptr};
    size_t num_terminals{0};
    musaStreamCaptureStatus status{musaStreamCaptureStatusNone};
  };

  CaptureInfo stream_get_capture_info(musaStream_t stream) {
    CaptureInfo info{};
    C10_MUSA_CHECK(musaStreamGetCaptureInfo(
        stream,
        &info.status,
        &info.capture_id,
        &info.graph,
        ...
        &info.num_terminals));
    TORCH_INTERNAL_ASSERT(
        info.status != musaStreamCaptureStatusInvalidated,
        "Invalid stream capture status");

    return info;
  }

  // Record "free marker" of the MUSA graph for all streams that
  ...
  std::vector<musaGraphNode_t> record_free_markers(Block* block) {
    // Is is possible to have the same marker recorded multiple times, so we use
    // a set to avoid duplicates
    ska::flat_hash_set<musaGraphNode_t> markers;
    musaGraph_t owning_graph = nullptr;

    auto try_record = [&](musaStream_t s) -> bool {
      auto info = stream_get_capture_info(s);
      if (info.status == musaStreamCaptureStatusNone) {
    ...
    return std::vector<musaGraphNode_t>(markers.begin(), markers.end());
  }
  ...
  void update_visited(
    ...
    auto get_parents =
      ...
      size_t count = 0;
      C10_MUSA_CHECK(musaGraphNodeGetDependencies(node, nullptr, &count));

      std::vector<musaGraphNode_t> out(count);
      if (count) {
        C10_MUSA_CHECK(musaGraphNodeGetDependencies(node, out.data(), &count));
        out.resize(count);
      }
      return out;
    };
  ...
  void free_safe_blocks_in_capture(
    ...
    if (info.status == musaStreamCaptureStatusNone || info.num_terminals == 0) {
      return;
    }
  ...
  ShareableHandle shareIpcHandle(Block* block) {
    ...
    if (!block->expandable_segment_) {
      ...
      ss.write(reinterpret_cast<const char*>(&handle), MUSA_IPC_HANDLE_SIZE);
    } else {
  ...
  std::vector<StreamSegmentSize> getExpandableSegmentSizes() const {
    std::lock_guard<std::recursive_mutex> lock(mutex);
  ...
  void createOrIncrefPool(
      MempoolId_t mempool_id,
      std::shared_ptr<MUSAAllocator> allocator) {
...
class NativeCachingAllocator : public MUSAAllocator {
 ...
 public:
  ...
  void createOrIncrefPool(
      c10::DeviceIndex device,
      MempoolId_t mempool_id,
      std::shared_ptr<MUSAAllocator> allocator) override {
    assertValidDevice(device);
    ...
    if (val.has_value()) {
      ...
      for (size_t i = 0; i < tokenizer.size(); i++) {
        ...
        if (key == "backend") {
          ...
          i++;
          if (tokenizer[i] == "musaMallocAsync") {
...
MemPool::MemPool(
    std::shared_ptr<MUSACachingAllocator::MUSAAllocator> allocator,
  ...
  MUSACachingAllocator::createOrIncrefPool(device_, id_, std::move(allocator));
  if (use_on_oom) {
    MUSACachingAllocator::setUseOnOOM(device_, id_, true);
  }
  if (no_split) {
    MUSACachingAllocator::setNoSplit(device_, id_);
  }
}
...
MemPool::~MemPool() {
  MUSACachingAllocator::setUseOnOOM(device_, id_, false);
