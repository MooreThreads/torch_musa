...
def _mkdir_p(d: str) -> None:
    try:
        ...
        raise RuntimeError(
            f"Failed to create folder {os.path.abspath(d)}: {error.strerror}"
        ) from error
...
class CMake:
    ...
    def get_cmake_cache_variables(self) -> Dict[str, CMakeValue]:
        ...
        """
        with open(self._cmake_cache_file, encoding="utf-8") as f:
    ...
    def generate(
        ...
        base_dir = os.getcwd()
        install_dir = os.path.join(base_dir, self.install_dir_prefix)
        ...
        # Build options that do not start with "BUILD_", "USE_", or "CMAKE_" and are directly
        # controlled by env vars. This is a dict that maps environment variables to the
        # corresponding variable name in CMake.
        additional_options = {
            # Key: environment variable name. Value: Corresponding variable name to be passed to
            # CMake. If you are adding a new build option to this block: Consider making these
            # two names identical and adding this option in the block below.
        ...
        additional_options.update(
            {
                # Build options that have the same environment variable name and CMake variable
                # name and that do not start with "BUILD_", "USE_", or "CMAKE_". If you are
        ...
        for var, val in my_env.items():
            # We currently pass over all environment variables that start with "BUILD_", "USE_",
            # and "CMAKE_". This is because we currently have no reliable way to get the list of
            # all build options we have specified in CMakeLists.txt. (`cmake -L` won't print
            # dependent options when the dependency condition is not met.) We will possibly
            # change this in the future by parsing CMakeLists.txt ourselves (then
            # additional_options would also not be needed to be specified here).
        ...
        # Some options must be post-processed. Ideally, this list will be shrunk to only one or
        # two options in the future, as CMake can detect many of these libraries pretty
        # comfortably. We have them here for now before CMake integration is completed. They
        # appear here not in the CMake.defines call below because they start with either "BUILD_"
        # or "USE_" and must be overwritten here.
        build_options.update(
            {
                # Note: Do not add new build options to this dict if it is directly read from
                # environment variable -- you only need to add one in `CMakeLists.txt`. All build
                # options that start with "BUILD_", "USE_", or "CMAKE_" are automatically passed
                # to CMake; For other options you can add to additional_options above.
                "BUILD_PYTHON": build_python,
                "BUILD_TEST": build_test,
                # Most library detection should go to CMake script, except this one, which Python
                # can do a much better job due to NumPy's inherent Pythonic nature.
                "USE_NUMPY": USE_NUMPY,
            }
        )
        ...
        # We set some CMAKE_* options in our Python build code instead of relying on the user's
        # direct settings. Emit an error if the user also attempts to set these CMAKE options
        # directly.
