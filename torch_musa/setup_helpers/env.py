...
class BuildType:
    """Checks build type. The build type will be given in :attr:`cmake_build_type_env`. If
    :attr:`cmake_build_type_env` is ``None``, then the build type will be inferred from
    ``CMakeCache.txt``. If ``CMakeCache.txt`` does not exist, os.environ['CMAKE_BUILD_TYPE'] will
    be used.

    Args: cmake_build_type_env (str): The value of os.environ['CMAKE_BUILD_TYPE']. If None,
    the actual build type will be inferred.
    ...
    def __init__(self, cmake_build_type_env: Optional[str] = None) -> None:
        ...
        if os.path.isfile(cmake_cache_txt) and not check_env_flag("DEBUG"):
            ...
            # Normally it is anti-pattern to determine build type from CMAKE_BUILD_TYPE because
            # it is not used for multi-configuration build tools, such as Visual Studio and
            # XCode. But since we always communicate with CMake using CMAKE_BUILD_TYPE from our
            # Python scripts, this is OK here.
...
# hotpatch environment variable 'CMAKE_BUILD_TYPE'. 'CMAKE_BUILD_TYPE' always prevails over DEBUG
# or REL_WITH_DEB_INFO.
