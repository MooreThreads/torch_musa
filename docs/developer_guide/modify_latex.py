...
class ModifyLatex:
    ...
    def modify_table_attributes(self):
        ...
        for match in m:
            ...
            if len(caption_dict ) > 0 :
                newtableattr = self._modify_single_table_attributes(tablestr[0] + tablestr[1] + tablestr[2], caption_dict)
    ...
    def _modify_single_table_attributes(self, singletablecontent, caption_dict):
        ...
        if tablecaption in caption_dict:
            ...
            new_singletablecontent = self._start_modify_table_attr(singletablecontent,
                                                                 tablestyle_dict['isLongTable'],
                                                                 tablestyle_dict['isCusHead'])
            if tablestyle_dict['isVertical']==True:
                new_singletablecontent = self._modify_vertical_table(new_singletablecontent)
        else:
            new_singletablecontent = self._start_modify_table_attr(singletablecontent, False)
    ...
    def _start_modify_table_attr(self, singletablecontent, islongtable, isCusHead = True):
        ...
        if splittable == None or len(splittable) < 5:
            ...
            newtable4 = self._modify_long_table_head(splittable[2], self.tables['headtype'])
    ...
    def _modify_first_column_type(self,cellcontent):
        ...
        if (r'\textbf' or r'\textcolor') in onelist:
            return cellcontent
        new_cellcontent = '\n' + r'\cellcolor' + self.tables['headtype'] + r'{' + fontcolor.replace('{}' , '{' + onelist+'}', 1) + r'}' + '\n'
