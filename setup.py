...
if not CLEAN_MODE:
    ...
    if pytorch_root == '':
        raise RuntimeError("Building error: PYTORCH_REPO_PATH must be set to PyTorch repository when building, but now it is empty!")
...
def build_musa_lib():
    ...
    cmake.generate(
        version, cmake_python_library, True, build_test, env, RERUN_CMAKE
    )
    cmake.build(env)
